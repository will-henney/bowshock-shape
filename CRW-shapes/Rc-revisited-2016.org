...
+ We actually want two things:
  ...
  2. [-] A finer grid so that we can fit functions to a(\xi, \beta) and x_0(\xi, \beta)
     ...
     + [ ] [2017-01-18 Wed] Jorge is doing fits to these
  3. [ ] Consider the error in \theta_1(\theta) for the head/tail fits, and use this to choose the angle \theta where we need to change between the head and tail approximations
     - [ ] Make graphs of the error - also serve to illustrate how good the fits are
