...
ax.plot(nn, Ribs, ls=":", label="Inner bow shock")
m = (tau > eta) & (tau < 1.0)
...
ax.text(1e-3, 4e-4, Mlabel, fontsize='small', bbox=box_params)
ax.text(1e-3, 7e-3, "Relative speed:\n$v = 40$ km/s", fontsize='small', bbox=box_params)
leg = ax.legend(fontsize="small")

ax.set(xscale='log', yscale='log')

ax.set(ylabel=r"Radius: $R$, pc", xlabel=r"Ambient density: $n$, cm$^{-3}$")
sns.despine()
fig.tight_layout()
fig.savefig(figname)
