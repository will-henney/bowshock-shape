...
#+BEGIN_SRC python :tangle bow_projection.py :eval no
  ...
  def theta_0(inc, func_R, *args_for_func_R):
      """Minimum theta, corresponding to y'_t = 0
      """

      tani = np.tan(inc)
      def f(theta):
          """Function to be zeroed: f(theta) = 0 for theta = theta_0"""
          om = omega(theta, func_R, *args_for_func_R)
          return np.sin(theta)*(1.0 - om*tani) - np.cos(theta)*(om + tani)

      # Root could be anywhere in range 0 -> pi, except not exactly pi
      th1, th2 = 0.0, np.pi - 1.e-6
      # Make sure we do indeed bracket the root
      assert(f(th1)*f(th2) <= 0.0, "Unbracketed root in theta_0!")
      # And use Brent's method to find the root
      return brentq(f, th1, th2)

  # Number of neighborhood points to use when fitting around projected
  # axes
  N_NEIGHBORHOOD = 50
  # Size of neighborhood in radians
  DTH_NEIGHBORHOOD = 0.01
  # Degree of polynomial used in fitting neighborhood
  DEGREE_POLY_NEIGHBORHOOD = 5
  ...
  prime', 'theta_90'
      ...
      th0 = theta_0(inc, func_R, *args_for_func_R)

      # Make a grid of theta in the neighborhood of th0
      # 
      # dth is 0.01 radians usually, but not more than half of range from
      # th0 -> pi
      dth = min(DTH_NEIGHBORHOOD, 0.5*(np.pi - th0))
      ...
      #
      # It may seem excessive to use deg=5, but this was necessary to
      # get 0.1% relative error in R'_c for the parabola at inc=89 degrees
