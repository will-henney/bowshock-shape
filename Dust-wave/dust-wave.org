...
:PROPERTIES:
:ID:       D2F2402D-A779-437C-AB92-84454229D6E6
:END:
...
**** Same but for a weak wind star
+ Inspired by sigma Ori
+ Like the 20 M_\odot case but with \eta = 0.0002
  + Maybe we will try 0.001 to be less extreme
  + This would be like the Martin:2005a stars
#+BEGIN_SRC python :eval no :tangle zones-v-n-plane-Weak.py 
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
  import seaborn as sns
...
#+BEGIN_SRC sh :results file
python zones-v-n-plane-Weak.py
#+END_SRC

#+RESULTS:
[[file:zones-v-n-plane-Weak.pdf]]
...
**** Same but for the Sickle Nebula 
+ B1.5V star, but with low velocity neutral stream

#+BEGIN_SRC python :eval no :tangle zones-v-n-plane-sickle.py 
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
  import seaborn as sns
...
#+BEGIN_SRC sh :results file
  python zones-v-n-plane-sickle.py
#+END_SRC

#+RESULTS:
[[file:zones-v-n-plane-sickle.pdf]]
*** Wind momentum ratio versus spectral type
**** Plot all the tables
#+header: :var TAB_B_K=B-krticka TAB_OV_K=O-V-krticka
#+header: :var TAB_OIII_K=O-III-krticka TAB_OI_K=O-I-krticka
#+header: :var TAB_B_KV1=B-krticka-vink-hot TAB_B_KV2=B-krticka-vink-cool
#+header: :var TAB_OV_KV=O-V-krticka-vink
#+header: :var TAB_OIII_KV=O-III-krticka-vink TAB_OI_KV=O-I-krticka-vink
#+header: :var TAB_OBV_S=OB-V-smith TAB_OBIII_S=OB-III-smith TAB_OBI_S=OB-I-smith
#+header: :var TAB_MM_V=MM-vink TAB_OV_MV=O-V-martins-vink
#+header: :var TAB_OIII_MV=O-III-martins-vink TAB_OI_MV=O-I-martins-vink
#+header: :colnames no
#+BEGIN_SRC python :results output verbatim :tangle eta-star-models.py
  import sys
  import numpy as np
...
#+BEGIN_SRC sh :results file
python eta-star-models.py
#+END_SRC

#+RESULTS:
[[file:eta-star-models.pdf]]
**** Magnetic and rotation effects
+ Puls:2008a has a good discussion
  + \eta_* = B^2 R_*^2 / Mdot V_w is the important parameter
  + Alfven radius: R_A = \eta_*^{1/4} R_* for \eta_* \gg 1
  + If \eta_* < 10, then total mass flux is not affected much
  + ud-Doula:2008a
**** Weak wind problem
+ Marcolino:2009a
...
**** Tables from Marcolino:2009a
Observations of weak line stars
| Star      | Sp type     | T        | log L         | Vw           | log Mdot      | log eta        |
|-----------+-------------+----------+---------------+--------------+---------------+----------------|
| HD 216898 | O9IV, O8.5V | 34 +/- 1 | 4.73 +/- 0.25 | 1700 +/- 500 | -9.35 +/- 0.7 | -3.16 +/- 0.75 |
| HD 326329 | O9V         | 31 +/- 1 | 4.74 +/- 0.1  | 1700 +/- 500 | -9.22 +/- 0.7 | -3.04 +/- 0.72 |
| HD 66788  | O8-9V       | 34 +/- 1 | 4.96 +/- 0.25 | 2200 +/- 500 | -8.92 +/- 0.7 | -2.84 +/- 0.75 |
| ζ Oph     | O9.5Vnn     | 32 +/- 2 | 4.86 +/- 0.10 | 1500 +/- 500 | -8.80 +/- 0.7 | -2.79 +/- 0.72 |
| HD 216532 | O8.5V((n))  | 33 +/- 2 | 4.79 +/- 0.25 | 1500 +/- 500 | -9.22 +/- 0.7 | -3.14 +/- 0.76 |
#+TBLFM: $7=($6 - $4) + log10($Msun ($5) $km $c / $Lsun $yr) ; f2::
**** Tables from Lucy:2010a
These are attempts to model weak wind stars
**** Table for \sigma Ori
| Source         | log L |    Mdot |   Vw |     eta |
|----------------+-------+---------+------+---------|
| Howarth:1989a  |   4.6 |    8e-8 | 1060 | 1.05e-1 |
| Repolust:2005a |  4.57 | 1.65e-8 | 2300 | 5.05e-2 |
| Najarro:2011a  |  4.71 |   2e-10 | 1500 | 2.89e-4 |
#+TBLFM: $5=$3 $Msun $4 $km $c / 10**$2 $Lsun $yr ;s3

+ Repolust:2005a is from H and K band lines (e.g., Br \gamma)
+ Najarro:2011a is from L band lines (e.g., Br \alpha), which supposedly is more reliable
  + There is a relationship between EW(Br \alpha) and Q' = Mdot / R^{3/2} T_eff^2 V_w

**** Tables from Krticka:2014a

+ B stars
#+name: B-krticka
| Model | ST |  Teff |     M | log L |  Mdot |    V |   Lw |     eta |
|-------+----+-------+-------+-------+-------+------+------+---------|
| T30   | B0 | 30000 | 14.73 |  4.39 |   210 | 3650 |  2.3 | 1.54e-2 |
| T28   |    | 28000 | 12.69 |  4.20 |   160 | 4380 |  2.5 | 2.19e-2 |
| T26   | B1 | 26000 | 10.88 |  3.99 |    96 | 3070 |  0.7 | 1.49e-2 |
| T24   |    | 24000 |  9.26 |  3.77 |    39 | 1700 | 9e-2 | 5.57e-3 |
| T22   |    | 22000 |  7.84 |  3.54 |   7.9 | 1690 | 2e-2 | 1.90e-3 |
| T20   |    | 20000 |  6.60 |  3.30 |   3.4 | 1290 | 5e-3 | 1.09e-3 |
| T18   |    | 18000 |  5.52 |  3.03 |  0.91 |  820 | 5e-4 | 3.44e-4 |
| T16   | B5 | 16000 |  4.58 |  2.75 | 0.072 |  240 | 3e-6 | 1.52e-5 |
| T14   | B6 | 14000 |  3.76 |  2.43 |     0 |    0 |  0.0 |     0e0 |
#+TBLFM: $8=$6 1e-11 $Msun ($7 $km)**2 / 2 $yr $Lsun ; f1::$9=$6 1e-11 $Msun $7 $km $c / $yr $Lsun 10**$5; s3


**** Krticka:2014a B stars, but using Vink:2001a equation
+ So we just use the (T, M, L) set from Krticka, and plug it in to the Vink equations
  + This is different for <25000 and >25000
+ Eddington ratio should be small:
  + \sigma_e = 6.6e-25 cm^2
  + \kappa = \sigma / m = 0.3 cm^2/g
  + L / M = 21.82 600 / 0.3 = 43640
+ First with the high T_eff formula and Vw = 2.6 Vesc
  #+name: B-krticka-vink-hot
  | Model | ST |  Teff |     M | log L |  Gamma | Vesc |    Vw |  Mdot |   Lw |     eta |
  |-------+----+-------+-------+-------+--------+------+-------+-------+------+---------|
  | T30   | B0 | 30000 | 14.73 |  4.39 | 0.0382 | 965. | 2509. | -8.06 | 4.52 | 4.40e-2 |
  | T28   |    | 28000 | 12.69 |  4.20 | 0.0286 | 938. | 2439. | -8.51 | 1.52 | 2.35e-2 |
  | T26   | B1 | 26000 | 10.88 |  3.99 | 0.0206 | 913. | 2374. | -9.03 | 0.43 | 1.12e-2 |
  | T24   |    | 24000 |  9.26 |  3.77 | 0.0146 | 886. | 2304. | -9.61 | 0.11 | 4.75e-3 |
  #+TBLFM: $6=(10**$5 / $4) / 43640 ; f4::$7=1070 ($3/1e4) ($4*(1 - $6))**0.5 / 10**($5/4) ; f0::$8=2.6 $-1 ;f0::$9=-6.697 + 2.195 ($5 - 5) - 1.313 log10($4/30) - 1.226 log10(2.6/2) + 0.933 log10($3/40000) - 10.92 (log10($3/40000))**2 ; f2::$10=10**$-1 $Msun ($-2 $km)**2 / 2 $yr $Lsun ;f2::$11=10**$9 $Msun $8 $km $c / $yr 10**$5 $Lsun ; s3
+ Second, with the low T_eff formula and Vw = 1.3 Vesc
  #+name: B-krticka-vink-cool
  | Model | ST |  Teff |    M | log L |  Gamma | Vesc |    Vw |   Mdot |     Lw |     eta | Tjump |
  |-------+----+-------+------+-------+--------+------+-------+--------+--------+---------+-------|
  | T22   |    | 22000 | 7.84 |  3.54 | 0.0101 | 855. | 1112. |  -8.79 |   0.17 | 2.57e-2 |  22.7 |
  | T20   |    | 20000 | 6.60 |  3.30 | 0.0069 | 820. | 1066. |  -9.26 |   0.05 | 1.45e-2 |  22.7 |
  | T18   |    | 18000 | 5.52 |  3.03 | 0.0044 | 789. | 1026. |  -9.81 |   0.01 | 7.34e-3 |  22.6 |
  | T16   | B5 | 16000 | 4.58 |  2.75 | 0.0028 | 751. |  976. | -10.37 | 3.4e-3 | 3.66e-3 |  22.6 |
  | T14   | B6 | 14000 | 3.76 |  2.43 | 0.0016 | 717. |  932. | -11.03 | 6.7e-4 | 1.60e-3 |  22.6 |
  #+TBLFM: $6=(10**$5 / $4) / 43640 ; f4::$7=1070 ($3/1e4) ($4*(1 - $6))**0.5 / 10**($5/4) ; f0::$8=1.3 $-1 ;f0::$9=-6.688 + 2.210 ($5 - 5) - 1.339 log10($4/30) - 1.601 log10(1.3/2) + 1.077 log10($3/20000) ; f2::$10=10**$9 $Msun ($8 $km)**2 / 2 $yr $Lsun ;f2::$11=10**$9 $Msun $8 $km $c / $yr 10**$5 $Lsun ; s3::$12=61.2 +2.59 (-14.9 + 3.2 $6) ;f1
+ Last column indicates that bi-stability jump should be between the 22 and 24 models



**** Tables from Krticka:2017a

O stars

Main sequence
#+name: O-V-krticka
|     T |    M | log L |   Mdot |   Vw |    Lw |     eta |
|-------+------+-------+--------+------+-------+---------|
| 30000 | 12.9 |  4.50 | 1.0e-8 | 2440 |   4.9 | 3.82e-2 |
| 32500 | 16.4 |  4.74 | 1.4e-8 | 1900 |   4.2 | 2.39e-2 |
| 35000 | 20.9 |  4.97 | 3.8e-8 | 1220 |   4.7 | 2.46e-2 |
| 37500 | 26.8 |  5.19 | 1.1e-7 | 1770 |  28.4 | 6.22e-2 |
| 40000 | 34.6 |  5.42 | 2.5e-7 | 1960 |  79.2 | 9.21e-2 |
| 42500 | 45.0 |  5.64 | 4.5e-7 | 1910 | 135.4 | 9.74e-2 |
#+TBLFM: $6=$4 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 $Msun $5 $km $c / $yr $Lsun 10**$3; s3

Giants
#+name: O-III-krticka
|     T |    M | log L |   Mdot |   Vw |    Lw |     eta |
|-------+------+-------+--------+------+-------+---------|
| 30000 | 19.3 |  5.10 | 6.2e-8 | 2070 |  21.9 | 5.04e-2 |
| 32500 | 22.8 |  5.25 | 1.2e-7 | 1470 |  21.4 | 4.91e-2 |
| 35000 | 27.2 |  5.41 | 2.4e-7 | 1200 |  28.5 | 5.54e-2 |
| 37500 | 32.5 |  5.57 | 4.2e-7 | 1620 |  90.9 | 9.06e-2 |
| 40000 | 39.2 |  5.71 | 9.3e-7 | 1320 | 133.7 | 1.18e-1 |
| 42500 | 47.4 |  5.85 | 1.5e-6 | 1200 | 178.2 | 1.26e-1 |
#+TBLFM: $6=$4 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 $Msun $5 $km $c / $yr $Lsun 10**$3; s3
...
|     T |    M | log L |   Mdot |   Vw |    Lw |     eta |
|-------+------+-------+--------+------+-------+---------|
| 30000 | 28.8 |  5.56 | 4.5e-7 | 1110 |  45.7 | 6.80e-2 |
| 32500 | 34.0 |  5.66 | 5.7e-7 | 1140 |  61.1 | 7.03e-2 |
| 35000 | 40.4 |  5.75 | 6.9e-7 | 1350 | 103.7 | 8.19e-2 |
| 37500 | 48.3 |  5.84 | 1.2e-6 | 1260 | 157.2 | 1.08e-1 |
| 40000 | 58.1 |  5.92 | 1.4e-6 | 1460 | 246.2 | 1.22e-1 |
| 42500 | 70.3 |  6.00 | 1.8e-6 | 1350 | 270.6 | 1.20e-1 |
#+TBLFM: $6=$4 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 $Msun $5 $km $c / $yr $Lsun 10**$3; s3


**** Tables from Krticka:2017a but using Vink:2001a mass loss recipe

Note that these use the Vink velocities still

O stars

Main sequence
#+name: O-V-krticka-vink
|     T |    M | log L |   Mdot |   Vw |    Lw |     eta |
|-------+------+-------+--------+------+-------+---------|
| 30000 | 12.9 |  4.50 | 1.3e-8 | 2440 |   6.4 | 4.96e-2 |
| 32500 | 16.4 |  4.74 | 4.2e-8 | 1900 |  12.5 | 7.18e-2 |
| 35000 | 20.9 |  4.97 | 1.2e-7 | 1220 |  14.7 | 7.76e-2 |
| 37500 | 26.8 |  5.19 |   3e-7 | 1770 |  77.5 | 1.70e-1 |
| 40000 | 34.6 |  5.42 | 7.3e-7 | 1960 | 231.4 | 2.69e-1 |
| 42500 | 45.0 |  5.64 | 1.6e-6 | 1910 | 481.5 | 3.46e-1 |
#+TBLFM: $6=$4 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 $Msun $5 $km $c / $yr $Lsun 10**$3; s3

Giants
#+name: O-III-krticka-vink
|     T |    M | log L |   Mdot |   Vw |    Lw |     eta |
|-------+------+-------+--------+------+-------+---------|
| 30000 | 19.3 |  5.10 | 1.6e-7 | 2070 |  56.6 | 1.30e-1 |
| 32500 | 22.8 |  5.25 | 3.7e-7 | 1470 |  66.0 | 1.51e-1 |
| 35000 | 27.2 |  5.41 | 8.0e-7 | 1200 |  95.0 | 1.85e-1 |
| 37500 | 32.5 |  5.57 | 1.5e-6 | 1620 | 324.8 | 3.24e-1 |
| 40000 | 39.2 |  5.71 | 2.7e-6 | 1320 | 388.1 | 3.44e-1 |
| 42500 | 47.4 |  5.85 | 4.5e-6 | 1200 | 534.6 | 3.77e-1 |
#+TBLFM: $6=$4 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 $Msun $5 $km $c / $yr $Lsun 10**$3; s3

Supergiants
#+name: O-I-krticka-vink
|     T |    M | log L |   Mdot |   Vw |    Lw |     eta |
|-------+------+-------+--------+------+-------+---------|
| 30000 | 28.8 |  5.56 | 9.9e-7 | 1110 | 100.6 | 1.50e-1 |
| 32500 | 34.0 |  5.66 | 1.7e-6 | 1140 | 182.3 | 2.10e-1 |
| 35000 | 40.4 |  5.75 | 2.6e-6 | 1350 | 390.9 | 3.09e-1 |
| 37500 | 48.3 |  5.84 | 3.7e-6 | 1260 | 484.6 | 3.33e-1 |
| 40000 | 58.1 |  5.92 | 4.7e-6 | 1460 | 826.5 | 4.08e-1 |
| 42500 | 70.3 |  6.00 | 5.6e-6 | 1350 | 842.0 | 3.74e-1 |
#+TBLFM: $6=$4 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 $Msun $5 $km $c / $yr $Lsun 10**$3; s3

**** Tables from Smith:2006a

+ Cites Prinja et al (1990) for V
+ Cites Repolust et al (2004) for Mdot

#+name: OB-V-smith
| ST   | LC | log L | Mdot |    V |     Lw |  eta |
|------+----+-------+------+------+--------+------|
| O3   | V  |  5.84 |  3.1 | 3160 | 2553.7 | 0.70 |
| O3.5 | V  |  5.76 |  2.5 | 3080 | 1956.5 | 0.66 |
| O4   | V  |  5.67 |  2.0 | 2990 | 1475.1 | 0.63 |
| O4.5 | V  |  5.58 |  1.6 | 2870 | 1087.2 | 0.60 |
| O5   | V  |  5.49 |  1.3 | 2760 |  817.0 | 0.57 |
| O5.5 | V  |  5.41 |  1.0 | 2650 |  579.3 | 0.51 |
| O6   | V  |  5.32 |  0.8 | 2560 |  432.5 | 0.48 |
| O6.5 | V  |  5.23 |  0.6 | 2460 |  299.5 | 0.43 |
| O7   | V  |  5.14 |  0.4 | 2320 |  177.6 | 0.33 |
| O7.5 | V  |  5.05 | 0.35 | 2210 |  141.0 | 0.34 |
| O8   | V  |  4.96 | 0.26 | 2100 |   94.6 | 0.30 |
| O8.5 | V  |  4.86 | 0.22 | 1960 |   69.7 | 0.29 |
| O9   | V  |  4.77 | 0.18 | 1820 |   49.2 | 0.28 |
| O9.5 | V  |  4.68 | 0.15 | 1660 |   34.1 | 0.26 |
| B0   | V  |  4.57 | 0.12 | 1510 |   22.6 | 0.24 |
| B0.5 | V  |  4.47 | 0.10 | 1460 |   17.6 | 0.24 |
| B1   | V  |  4.37 | 0.08 | 1180 |    9.2 | 0.20 |
| B1.5 | V  |  4.28 | 0.06 |  960 |    4.6 | 0.15 |
| B2   | V  |  4.19 | 0.05 |  750 |    2.3 | 0.12 |
#+TBLFM: $6=$4 1e-6 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 1e-6 $Msun $5 $km $c / $yr $Lsun 10**$3; f2

#+name: OB-III-smith
| ST   | LC  | log L | Mdot |    V |     Lw |  eta |
|------+-----+-------+------+------+--------+------|
| O3   | III |  5.96 |  6.5 | 3160 | 5354.6 | 1.11 |
| O3.5 | III |  5.91 |  5.5 | 3080 | 4304.3 | 1.03 |
| O4   | III |  5.85 |  4.7 | 2990 | 3466.4 | 0.98 |
| O4.5 | III |  5.79 |  4.0 | 2870 | 2718.1 | 0.92 |
| O5   | III |  5.73 |  3.4 | 2760 | 2136.7 | 0.86 |
| O5.5 | III |  5.67 |  2.7 | 2650 | 1564.2 | 0.76 |
| O6   | III |  5.61 |  2.2 | 2560 | 1189.4 | 0.68 |
| O6.5 | III |  5.54 |  1.8 | 2460 |  898.6 | 0.63 |
| O7   | III |  5.48 |  1.5 | 2320 |  666.0 | 0.57 |
| O7.5 | III |  5.42 |  1.2 | 2210 |  483.5 | 0.50 |
| O8   | III |  5.35 |  0.8 | 2100 |  291.0 | 0.37 |
| O8.5 | III |  5.28 |  0.7 | 1960 |  221.8 | 0.36 |
| O9   | III |  5.21 |  0.5 | 1820 |  136.6 | 0.28 |
| O9.5 | III |  5.15 |  0.4 | 1660 |   90.9 | 0.23 |
| B0   | III |  5.08 |  0.3 | 1510 |   56.4 | 0.19 |
| B0.5 | III |  5.00 | 0.25 | 1460 |   44.0 | 0.18 |
| B1   | III |  4.93 |  0.2 | 1180 |   23.0 | 0.14 |
| B1.5 | III |  4.86 | 0.15 |  960 |   11.4 | 0.10 |
| B2   | III |  4.78 | 0.11 |  750 |    5.1 | 0.07 |
#+TBLFM: $6=$4 1e-6 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 1e-6 $Msun $5 $km $c / $yr $Lsun 10**$3; f2
...
| ST   | LC | log L | Mdot |    V |      Lw |  eta |
|------+----+-------+------+------+---------+------|
| O3   | I  |  5.99 | 13.0 | 3160 | 10709.1 | 2.08 |
| O3.5 | I  |  5.96 | 11.3 | 3080 |  8843.4 | 1.89 |
| O4   | I  |  5.93 | 10.0 | 2990 |  7375.3 | 1.74 |
| O4.5 | I  |  5.90 |  8.9 | 2870 |  6047.7 | 1.59 |
| O5   | I  |  5.87 |  8.0 | 2760 |  5027.4 | 1.47 |
| O5.5 | I  |  5.84 |  7.1 | 2650 |  4113.3 | 1.35 |
| O6   | I  |  5.81 |  6.4 | 2560 |  3460.2 | 1.26 |
| O6.5 | I  |  5.78 |  5.7 | 2460 |  2845.7 | 1.15 |
| O7   | I  |  5.75 |  5.0 | 2320 |  2220.2 | 1.02 |
| O7.5 | I  |  5.72 |  4.3 | 2210 |  1732.6 | 0.90 |
| O8   | I  |  5.68 |  3.6 | 2100 |  1309.7 | 0.78 |
| O8.5 | I  |  5.65 |  3.0 | 1960 |   950.8 | 0.65 |
| O9   | I  |  5.61 |  2.5 | 1820 |   683.2 | 0.55 |
| O9.5 | I  |  5.57 |  1.9 | 1660 |   431.9 | 0.42 |
#+TBLFM: $6=$4 1e-6 $Msun ($5 $km)**2 / 2 $yr $Lsun ; f1::$7=$4 1e-6 $Msun $5 $km $c / $yr $Lsun 10**$3; f2


**** Tables from Vink:2018a

Ridiculously high mass stars, all at 15,000 K, all approaching Edd limit

I am only doing the solar metallicity ones

#+name: MM-vink
|   M | log L |   Vw | log Mdot |     Lw |  eta |
|-----+-------+------+----------+--------+------|
| 100 |  6.11 |  635 |    -4.97 |  356.4 | 0.26 |
| 100 |  6.11 | 1548 |    -5.72 |  376.7 | 0.11 |
| 193 |  6.56 |  610 |    -4.18 | 2028.1 | 0.55 |
| 489 |  7.11 |  298 |    -3.19 | 4730.1 | 0.74 |
| 550 |  7.17 |  111 |    -3.21 |  626.7 | 0.23 |
#+TBLFM: $5=10**$4 $Msun ($3 $km)**2 / 2 $yr $Lsun ; f1::$6=10**$4 $Msun $3 $km $c / $yr $Lsun 10**$2; f2


**** Martins:2005a O stars with Vink:2000a wind equation

Class V

#+name: O-V-martins-vink
| ST   |     T | log L |     R |     M |  Gamma |  Vesc | 2.6 Vesc |  Mdot |     Lw |    eta |
|------+-------+-------+-------+-------+--------+-------+----------+-------+--------+--------|
| O3   | 44616 |  5.83 | 13.84 | 58.34 | 0.2656 | 1090. |    2834. | -5.37 | 2826.4 | 0.8845 |
| O4   | 43419 |  5.68 | 12.31 | 46.16 | 0.2376 | 1048. |    2725. | -5.57 | 1648.8 | 0.7580 |
| O5   | 41540 |  5.51 | 11.08 | 37.28 | 0.1989 | 1018. |    2647. | -5.83 |  855.0 | 0.5985 |
| O5.5 | 40062 |  5.41 | 10.61 | 34.17 | 0.1724 | 1012. |    2631. | -6.01 |  558.1 | 0.4948 |
| O6   | 38151 |  5.30 | 10.23 | 31.73 | 0.1441 | 1007. |    2618. | -6.23 |  332.9 | 0.3822 |
| O6.5 | 36826 |  5.20 |  9.79 | 29.02 | 0.1251 |  995. |    2587. | -6.43 |  205.1 | 0.3000 |
| O7   | 35531 |  5.10 |  9.37 | 26.52 | 0.1088 |  981. |    2551. | -6.62 |  128.8 | 0.2404 |
| O7.5 | 34419 |  5.00 |  8.94 | 24.15 | 0.0949 |  968. |    2517. | -6.82 |   79.1 | 0.1884 |
| O8   | 33383 |  4.90 |  8.52 | 21.95 | 0.0829 |  955. |    2483. | -7.02 |   48.6 | 0.1477 |
| O8.5 | 32522 |  4.82 |  8.11 | 19.82 | 0.0764 |  929. |    2415. | -7.17 |   32.5 | 0.1222 |
| O9   | 31524 |  4.72 |  7.73 | 18.03 | 0.0667 |  914. |    2376. | -7.37 |   19.9 | 0.0955 |
| O9.5 | 30488 |  4.62 |  7.39 | 16.46 | 0.0580 |  899. |    2337. | -7.59 |   11.6 | 0.0713 |
#+TBLFM: $6=(10**$3 / $5) / 43640 ;f4::$7=1070 ($2/1e4) ($5*(1 - $6))**0.5 / 10**($3/4) ; f0::$8=2.6 $-1;f0::$9=-6.697 + 2.195 ($3 - 5) - 1.313 log10($5/30) - 1.226 log10(2.6/2) + 0.933 log10($2/40000) - 10.92 (log10($2/40000))**2 ; f2::$10=10**$-1 $Msun ($-2 $km)**2 / 2 $yr $Lsun ;f1::$11=10**$9 $Msun $8 $km $c / $yr 10**$3 $Lsun ; f4

Class III
#+name: O-III-martins-vink
| ST   |     T | log L |     R |     M |  Gamma | Vesc | 2.6 Vesc |  Mdot |     Lw |    eta |
|------+-------+-------+-------+-------+--------+------+----------+-------+--------+--------|
| O3   | 42942 |  5.92 | 16.57 | 58.62 | 0.3251 | 957. |    2488. | -5.18 | 3373.9 | 0.9775 |
| O4   | 41486 |  5.82 | 15.83 | 48.80 | 0.3102 | 903. |    2348. | -5.30 | 2279.5 | 0.8810 |
| O5   | 39507 |  5.70 | 15.26 | 41.48 | 0.2769 | 870. |    2262. | -5.49 | 1365.9 | 0.7224 |
| O5.5 | 38003 |  5.63 | 15.13 | 38.92 | 0.2512 | 859. |    2233. | -5.63 |  964.3 | 0.6070 |
| O6   | 36673 |  5.56 | 14.97 | 36.38 | 0.2287 | 847. |    2202. | -5.77 |  679.3 | 0.5095 |
| O6.5 | 35644 |  5.49 | 14.74 | 33.68 | 0.2103 | 834. |    2168. | -5.90 |  488.2 | 0.4369 |
| O7   | 34638 |  5.43 | 14.51 | 31.17 | 0.1979 | 814. |    2116. | -6.02 |  352.8 | 0.3714 |
| O7.5 | 33487 |  5.36 | 14.34 | 29.06 | 0.1806 | 799. |    2077. | -6.17 |  240.6 | 0.3032 |
| O8   | 32573 |  5.30 | 14.11 | 26.89 | 0.1700 | 779. |    2025. | -6.29 |  173.5 | 0.2575 |
| O8.5 | 31689 |  5.24 | 13.88 | 24.84 | 0.1603 | 758. |    1971. | -6.41 |  124.7 | 0.2183 |
| O9   | 30737 |  5.17 | 13.69 | 23.07 | 0.1469 | 744. |    1934. | -6.56 |   85.0 | 0.1781 |
| O9.5 | 30231 |  5.12 | 13.37 | 21.04 | 0.1436 | 721. |    1875. | -6.65 |   64.9 | 0.1575 |
#+TBLFM: $6=(10**$3 / $5) / 43640 ;f4::$7=1070 ($2/1e4) ($5*(1 - $6))**0.5 / 10**($3/4) ; f0::$8=2.6 $-1;f0::$9=-6.697 + 2.195 ($3 - 5) - 1.313 log10($5/30) - 1.226 log10(2.6/2) + 0.933 log10($2/40000) - 10.92 (log10($2/40000))**2 ; f2::$10=10**$-1 $Msun ($-2 $km)**2 / 2 $yr $Lsun ;f1::$11=10**$9 $Msun $8 $km $c / $yr 10**$3 $Lsun ; f4

Class I
#+name: O-I-martins-vink
| ST   |     T | log L |     R |     M |  Gamma | Vesc | 2.6 Vesc |  Mdot |     Lw |    eta |
|------+-------+-------+-------+-------+--------+------+----------+-------+--------+--------|
| O3   | 42551 |  6.00 | 18.47 | 66.89 | 0.3426 | 955. |    2483. | -5.08 | 4230.5 | 1.0216 |
| O4   | 40702 |  5.94 | 18.91 | 58.03 | 0.3439 | 880. |    2288. | -5.14 | 3128.6 | 0.9413 |
| O5   | 38520 |  5.87 | 19.48 | 50.87 | 0.3339 | 818. |    2127. | -5.25 | 2098.8 | 0.7981 |
| O5.5 | 37070 |  5.82 | 19.92 | 48.29 | 0.3135 | 801. |    2083. | -5.35 | 1598.9 | 0.6966 |
| O6   | 35747 |  5.78 | 20.33 | 45.78 | 0.3016 | 776. |    2018. | -5.44 | 1219.8 | 0.6015 |
| O6.5 | 34654 |  5.74 | 20.68 | 43.10 | 0.2922 | 752. |    1955. | -5.52 |  952.2 | 0.5314 |
| O7   | 33326 |  5.69 | 21.14 | 40.91 | 0.2743 | 734. |    1908. | -5.64 |  688.0 | 0.4414 |
| O7.5 | 31913 |  5.64 | 21.69 | 39.17 | 0.2554 | 717. |    1864. | -5.78 |  475.7 | 0.3505 |
| O8   | 31009 |  5.60 | 22.03 | 36.77 | 0.2481 | 695. |    1807. | -5.87 |  363.4 | 0.3029 |
| O8.5 | 30504 |  5.58 | 22.20 | 33.90 | 0.2570 | 660. |    1716. | -5.89 |  312.9 | 0.2876 |
| O9   | 29569 |  5.54 | 22.60 | 31.95 | 0.2487 | 639. |    1661. | -6.00 |  227.6 | 0.2370 |
| O9.5 | 28430 |  5.49 | 23.11 | 30.41 | 0.2329 | 623. |    1620. | -6.15 |  153.3 | 0.1836 |
#+TBLFM: $6=(10**$3 / $5) / 43640 ;f4::$7=1070 ($2/1e4) ($5*(1 - $6))**0.5 / 10**($3/4) ; f0::$8=2.6 $-1;f0::$9=-6.697 + 2.195 ($3 - 5) - 1.313 log10($5/30) - 1.226 log10(2.6/2) + 0.933 log10($2/40000) - 10.92 (log10($2/40000))**2 ; f2::$10=10**$-1 $Msun ($-2 $km)**2 / 2 $yr $Lsun ;f1::$11=10**$9 $Msun $8 $km $c / $yr 10**$3 $Lsun ; f4

Save these tables to FITS format 
#+header: :var TAB_V=O-V-martins-vink TAB_III=O-III-martins-vink TAB_I=O-I-martins-vink 
#+BEGIN_SRC python :colnames no :result file :return path
  from astropy.table import Table
  path = '/Users/will/Dropbox/dust-wave-case-studies/data/Mdot_tables'
  for stab, tab in ['V', TAB_V], ['III', TAB_III], ['I', TAB_I]:
      Table(rows=tab[1:], names=tab[0]).write(
          f'{path}/martins-vink-O-{stab}.fits', overwrite=True)
#+END_SRC

#+RESULTS:
: /Users/will/Dropbox/dust-wave-case-studies/data/Mdot_tables
...
**** TODO Decoupling revisited - breakdown radius
+ We will take the result from the Cloudy models for the regimes of G/n where decoupling occurs
  + G/n = (1-3)e4 for soft spectra (MS10 and BSG)
  + G/n = 1e5 for hard spectra (MS20 and MS40)


#+BEGIN_SRC python :eval no :tangle coulomb-decouple-v-n-plane.py 
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
  import seaborn as sns
...
#+BEGIN_SRC sh :results file
python coulomb-decouple-v-n-plane.py
#+END_SRC

#+RESULTS:
[[file:coulomb-decouple-v-n-plane.pdf]]


+ [2018-05-05 Sat] Now revisited and simplified (see handwritten notes)
  + The boundaries are rather liberal, since they son't take into account grain inertia (or B-field) 
+ How do we find the dust-wave radius in general
  + Previously, we had used R_0 / (1 + \alpha)
  + But that can only work when the drag velocity smoothly increases with decreasing distance
+ We can work out the \alpha at the dust-free bow shock radius
  + For dust-wave separation, it is necessary that v < w_drift there, but the v may have been decelerated by passing through the dust, so may be less than v_\infty
...
+ We can also find g = G M / R^2 = G M T^4 4 pi sigma / L
  + for 1 Msun, 1 Lsun, and T = 1e4 this gives 2.476e5 cm/s^2
  + So g = 2.476e5 (T/1e4)^4 M / L
|        L |  M |     T |   Mdot | Vcrit |  Gamma_e |  V_esc | 2.6 V_esc | Check | log g |
|----------+----+-------+--------+-------+---------+-------+----------+-------+-------|
|    6.3e3 | 10 | 25200 | -9.473 |   670 |   0.017 |  949. |    2467. | 1.002 |  4.20 |
|  5.453e4 | 20 | 33872 | -7.308 |   721 | 0.07246 | 1022. |    2657. | 1.002 |  4.08 |
|  2.219e5 | 40 | 44124 | -6.221 |   898 |   0.147 | 1271. |    3305. | 1.001 |  4.23 |
| 3.4095e5 | 40 | 36290 | -5.890 |   607 |    0.24 |  886. |    2304. | 1.032 |  3.70 |
#+TBLFM: $4=-6.697 + 2.195 log10($1/1e5) - 1.313 log10($2/30) - 1.226 log10(2.6/2) + 0.933 log10($3/40000) - 10.92 (log10($3/40000))**2 ; f3::$7=1070 ($3/1e4) ($2*(1 - $6))**0.5 / $1**0.25 ; f0::$8=2.6 $-1 ; f0::$9=$7/$5 sqrt(2) ; f3::$10=log10(2.476e5 ($3/1e4)**4 $2 / $1);f2
...
* Revisiting dust waves

** Summary of Ochsendorf arguments
+ Finds that Coulomb force would spoil the dust wave
  + Therefore rejects Coulomb force!
  + This requires grain potential to be much lower
    + [ ] *Work out how much lower is required*
  + Suggests photoelectric yields may be lower than thought, but this doesn't seem a good argument
+ Has figure showing drag force versus velocity, similar to ours
+ Calculates dust temperature as function of spectral type
  + Finds it to fall with increasing stellar luminosity
  + [ ] *We should do the same for our models*
    + I get that
      + RBS: T ~ constant
      + RBW: T ~ \tau^{-1/3}
      + RBS: T ~ \eta^{-1/6}
    + But also T ~ (\rho v^2)^{1/6}
* 
* Cloudy models of grain charging
+ We want to find the grain potential as a function of ionization parameter
+ Plan: run some constant-density cloudy models for relevant parameters
+ This is done in [[file:~/Dropbox/cloudy-dust-charging/cloudy-dust-charging.org][file:~/Dropbox/cloudy-dust-charging/cloudy-dust-charging.org]]
** Dependence on parameters
+ We find two regions of highish drift velocity
  1. High-ionization region where we have coulomb breakdown
     - \varphi \approx 10
     - Radiation force exceeds maximum Coulomb drag => jump to the Rayleigh drag solution, which is at very high drift speed
     - Although drift speed is not meaningful here, instead should find terminal velocity
     - This occurs at about 1% of ifront radius and only for the B stars.  Not at all for the O stars
  2. Low-ionization region where drain potential is low, |\varphi| < 1
     - Occurs at neutral fraction of 0.001 to 0.01 for the O stars
     - In the ionization front for the B stars, but only at low density
     - The drift velocity only gets up to 0.5 \to 1 km/s, so it doesn't decouple much.
+ Ochsendorf gets round this by just saying that the grains are not charged as much as models suggest!
+ So for region 1, we would have decoupled dust waves, with \alpha_drag < 1
+ But for region 2, we would have \alpha_drag \ge 40 or so, which is the regime where we are close to the well-coupled regime, with substantial back-reaction on the gas
** Analytic calculation of G/n
+ Weingartner & Draine (2001) Fig 5, show that the yields are roughly unity in EUV for small grains, and about 0.1 for larger grains (> 300 Ang = 0.03 micron)
  + They are smaller in FUV below about 9 eV or 0.66 Ryd
  + The bulk yield is about 2 times smaller than the 300 Ang value
+ Radiation field is normalized to the Habing value:
  + energy density u_Hab = 5.33 \times 10^-14 erg cm^-3
  + mean intensity J_Hab = u c / 4 pi = 1.27e-4 erg cm^-2 s^-1 sr^-1
  + Flux is F_Hab = 4 \pi J_Hab = 1.6e-3 erg cm^-2 s^-1
    + [X] This is consistent with what I wrote in the Adal Carina paper
+ We will normalize to L = 1e4 Lsun and 1 pc
  + Nominal flux is F_0 = 0.31926 erg cm^-2 s^-1
  + Mean intensity is F_0 /4 pi for monodirectional radiation
  + J_0 = 0.0254 erg cm^-2 s^-1 sr^-1
  + G_0 = J_0 / J_Hab = 200
+ So if f_F and f_E are the fractions of bolometric luminosity in two bands
  + And \tau_F and \tau_E are the optical depths
  + Then G = G_0 L_4 R_pc^-2 [f_F exp(-\tau_F) + f_E exp(-\tau_E)]
  + We could maybe multiply the EUV term by a boost B_E to account for the higher yield
+ [X] Check this is consistent with WD sec 7.1, where they found G/n = 1.0 n^{1/3}
  + L = 4.4e38 = 1.1e5 Lsun => L_4 = 11
  + S = 3.6e48 => R_S = 1.4896e20 cm = 48 n^{-2/3} pc
  + G = 200 11 / (0.8 48)**2 = 1.49 n^{4/3 }
  + So it is consistent - small difference is probably with the recomb rate
+ So, for MS20 model
  + L_4 = 5, f_F = 0.64, f_E = 0.16
  + => G = 200 5 0.64 R_pc^-2 = 640 R_pc^-2
  + G T^{1/2}/n = 6.4e4 R_pc^-2 n^-1 if we ignore extinction
  + So with n = 1pcc at R = 0.1 pc, we have 6.4e6
    + Fig 10 of WD01 at  G T^{1/2}/n = 1e6 gives U = 5 V for silicate and 8 V for graphite 
    + dustrad-n00-MS20 gives potential of more like 12 V, but this is probably because we also have the EUV, not counted in G
    + Assuming \sigma_H = 1.6e-21, then \tau = 0.005 R_pc
    + But at 50 pc grains are negative
      + \tau = 0.24, so not so big
      + G T^{1/2}/n = 6.4e4 exp(-0.24) / 50**2 = 20, or G/n = 0.2
    + dustrad-n04-MS20 at 0.001 pc should also have charging param = 6.4e6
      + The 100x smaller radius exactly compensates for the 10,000x higher density
      + That at least is consistent
      + In this case, we don't go negative near the i-front
        + R = 0.06 pc => \tau = 1.85
        + G T^{1/2}/n = 6.4e4 exp(-1.85) / 0.06**2 1e4 = 280
        + This is 10x bigger than n=1 case, so unsurprising more positive charge 
+ Fig 11 gives the charging parameter G T^{1/2}/n for which grains are neutral
  + For 1e4 K this is about 1000 for all grains => G/n = 10
  + That assumes an EUV cutoff - if we add back in the EUV, then it probably goes down to G/n = 1 for neutral grains (based on discussion in 7.1)
+ [ ] It is still puzzling that the grain charge falls as much as it does in the MMS20 and MS40 models
  + We can estimate U for these models:
    + If we gas ionization parameter: X = x^2 / (1 - x), then the radiation ionization parameter: U \equiv F / c n is U = (\alpha_B / c \sigma) X
    + If we assume 1e4 K and \sigma = 0.5 \sigma_0 = 3e-18 cm^2, then
      + U = 2.9e-6 X
    + So we have U \ge 1 at the illuminated face, where X \approx 1e6
    + For the 40 Msun star f_F = 0.43, f_E = 0.45, L_4 = 22.2
      + So, G = 200 22.2 0.43 / R_pc^2 = 1910 / R_pc^2 
      + Inner face :: X = 1e6, U = 3 at R = 0.02 pc for n = 1
        + => G = 1910 / 0.02**2 = 4.8e6
          + G T^{1/2 }/ n = 4.8e8 => /far higher than considered in Weingartner 2001/
        + Potential is U = 10 eV => \phi = 10 / 0.86 = 11.6 => Q_drag = 19 \phi^2 = 2600 
        + Radiation pressure is L / 4 \pi R^2 c = 5.9e-7 erg/cm^3 
        + Gas pressure is 2 n k T = 2.761e-12 erg/cm^3 = 2e5 times smaller
      + 


* Plan for dust wave papers
+ Current plan is to split up the dust wave paper (II + III), but not sure at what point
+ Ingredients
  + Parameter space in strong coupling limit
    + Sec 2.1 of Paper II
  + Breakdown of strong coupling
    + Decoupling efficiency \xi = 2 Q_p / Q_drag
    + Parameter space with constant \xi
    + Dependence of \xi on grain potential \phi
    + Dependence of \phi on ionization parameter
  + Magnetic field effects
    + Larmor radius
    + Estimates of magnetic field strength - H II regions and diffuse 
  + Shapes of different bows
    + Dust waves
      + Non-magnetic - dragoids - not realistic, but a stepping stone to both bow waves and lorentzoids
      + Magnetic - lorentzoids - leave until next paper?
        + Resolved and unresolved Larmor radius
    + Radiation bow waves
      + Like dust waves, but with shocks (and without drag)
      + Numerical solution
        + Adapt something like pyro http://zingale.github.io/pyro2/intro.html
    + Radiation bow shocks
      + Should be Wilkinoid/Cantoid when optically thick - I think
      + How optical depth varies with \theta
        + I think it tends to increase, which means that wings become optically thick first
        + For instance, for Wilkinoid we have
          + \theta = 0 :: \sigma / (\rho_0 R_0) = 3/4
          + \theta = 90 :: \sigma / (\rho_0 R_0) = R_90^4 / 2 R_90 [ (\pi/2)^2 + (R_90^2 - 1)^2 ]^{1/2}
            + where R_90 = 3^{1/2}
            + => 3 3^{1/2} / 2 ( (\pi/2)^2 + 4 )^{1/2 } = 1.0216
            + In other words, very little difference: 0.75 \to 1
            + Wilkin says that this is perpendicular to shell, so would have to be multiplied by sec \alpha_90, but that is only modest 
** 
...
** Revisiting the core-halo grain models
+ Supposedly, timescale for mantle processing is 1e6/ G_0 years
  + This should steepen NIR-visible extinction
  + Modify IR emission bands
  + Enhance Mid-IR to mm emission
  + Apparently, Orion type dust is a result of photo-processing of previously coagulated dust
+ If the small grains have a sufficiently steep power index, then it
  may be the very smallest that dominate FUV extinction
...
#+BEGIN_SRC sh :results output
  FIGFILES="dust-couple-stream-annotate.pdf dust-wave-Rc-R90*.pdf dust-coupling-1d.pdf dust-divergent.pdf dust-couple-div-stream.jpg decouple-v-n-plane.pdf zones-v-n-plane.pdf zones-v-n-plane-?SG.pdf decouple-v40-versus-n.pdf multi-view-dust-bfield-?5.pdf dust-bfield-stream-b?5-z0000.jpg dust-lorentz-stream-3d-turb-b90-L2000-Ma0100-Z+000.jpg dust-lorentz-stream-3d-turbz-b90-L2000-Ma0100-Y+000.jpg"
  date
  pwd
...
multi-view-dust-bfield-05.pdf -> ../papers/Papers1234/figs/multi-view-dust-bfield-05.pdf
multi-view-dust-bfield-15.pdf -> ../papers/Papers1234/figs/multi-view-dust-bfield-15.pdf
multi-view-dust-bfield-45.pdf -> ../papers/Papers1234/figs/multi-view-dust-bfield-45.pdf
multi-view-dust-bfield-75.pdf -> ../papers/Papers1234/figs/multi-view-dust-bfield-75.pdf
dust-bfield-stream-b05-z0000.jpg -> ../papers/Papers1234/figs/dust-bfield-stream-b05-z0000.jpg
dust-bfield-stream-b15-z0000.jpg -> ../papers/Papers1234/figs/dust-bfield-stream-b15-z0000.jpg
dust-bfield-stream-b45-z0000.jpg -> ../papers/Papers1234/figs/dust-bfield-stream-b45-z0000.jpg
dust-bfield-stream-b75-z0000.jpg -> ../papers/Papers1234/figs/dust-bfield-stream-b75-z0000.jpg
dust-lorentz-stream-3d-turb-b90-L2000-Ma0100-Z+000.jpg -> ../papers/Papers1234/figs/dust-lorentz-stream-3d-turb-b90-L2000-Ma0100-Z+000.jpg
dust-lorentz-stream-3d-turbz-b90-L2000-Ma0100-Y+000.jpg -> ../papers/Papers1234/figs/dust-lorentz-stream-3d-turbz-b90-L2000-Ma0100-Y+000.jpg
#+end_example
...
** [1/1] Dust grain trajectories with magnetic field
...
+ [X] Improve these figures, inspired by the movies
  + Show 5 bursts of grains at t = -4, -2, 0, 2, 4 with respect to t = X0 - 1
  + Have them different colors, so we can appreciate the different trajectories
...
#+BEGIN_SRC sh :results file
python dust-bfield-stream.py 15.0 0.0 
#+END_SRC

#+RESULTS:
[[file:dust-bfield-stream-b15-z0000.jpg]]
...
+ Run this in shell for 05, 15, 30, 45, 60, 75
...
*** TODO [2/3] Apparent shapes of magnetized dust waves
...
#+BEGIN_SRC sh :results silent
python stitch-bfield-cube.py 45
#+END_SRC


**** DONE Projection as function of two angles 
CLOSED: [2018-01-11 Thu 20:20]
+ [X] This can be done with the ~scipy.interpolate.interpn~ function
+ [X] Write out a WCS to the fits images
+ [X] Do for various angles
  + Azimuth: 15, 45, 75 should cover everything modulo reflection symmetry
  + Inclination: four equally spaced in sin i
    |     | sin(i) | i, deg |
    |-----+--------+--------|
    | 0.5 |  0.125 |     7. |
    | 1.5 |  0.375 |    22. |
    | 2.5 |  0.625 |    39. |
    | 3.5 |  0.875 |    61. |
    #+TBLFM: $2=$1/4;f3::$3=arcsin($2);f0
    + So, ~inclinations = 7, 22, 39, 61~, ~azimuths = 15, 45, 75~
+ [ ] Make some figures
  + 4 x 3 grid of inclination x azimuth
  + show where the star is
  + show direction of B-field
...
***** Runs for grid of angles
+ Run in shell for id = dust-bfield-b05, b15, b30, b45, b60, b75
#+BEGIN_SRC sh :eval no :tangle multi-project-cube.sh
  id=$1
...
***** Look at a whole bunch at once
#+BEGIN_SRC sh :results silent
open dust-bfield-b30-cube-F-0{07,22,39,61}-0{15,45,75}.fits
#+END_SRC


***** Individual runs
#+BEGIN_SRC sh :results file
python project-cube-inc-azi.py dust-bfield-b30-cube-F 0 0
#+END_SRC

#+RESULTS:
[[file:dust-bfield-b30-cube-F-000-000.fits]]
...
#+BEGIN_SRC sh :results file
python project-cube-inc-azi.py dust-bfield-b30-cube-F 61 75
#+END_SRC

#+RESULTS:
[[file:dust-bfield-b30-cube-F-061-075.fits]]

***** [5/5] Figures of the view angle grids
+ [X] Basic images
+ [X] Labels with angles
+ [X] Indicate projected flow direction
  + Things seem to get rotated at high azimuth
  + Fixed that by changing way azimuth is done
+ [X] Indicate projected B direction
  + Have a circle with an arrow
+ [X] [2018-01-15 Mon] Generalize so that we can use it with the finite Larmor runs
#+BEGIN_SRC python :eval no :tangle multi-view-dust-bfield.py
  import sys
  import numpy as np
  from astropy.io import fits
...
#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 30
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-30.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 5
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-05.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 60
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-60.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 15
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-15.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 75
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-75.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-45.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L0200-Ma0100-alpha0010
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L0200-Ma0100-alpha0010.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L0500-Ma0100-alpha0010
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L0500-Ma0100-alpha0010.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L2000-Ma0100-alpha0010
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L2000-Ma0100-alpha0010.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L2000-Ma0020-alpha0010
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L2000-Ma0020-alpha0010.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L0200-Ma0020-alpha0010
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L0200-Ma0020-alpha0010.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 05 lorentz-stream -L2000-Ma0100-alpha0100
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b05-L2000-Ma0100-alpha0100.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L2000-Ma0100-alpha0100
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L2000-Ma0100-alpha0100.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L2000-Ma0100-alpha0050
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L2000-Ma0100-alpha0050.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L2000-Ma0100-alpha0200
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L2000-Ma0100-alpha0200.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 45 lorentz-stream -L2000-Ma0100-alpha0400
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b45-L2000-Ma0100-alpha0400.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 15 lorentz-stream -L2000-Ma0100-alpha0050
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b15-L2000-Ma0100-alpha0050.pdf]]

#+BEGIN_SRC sh :results file
python multi-view-dust-bfield.py 75 lorentz-stream -L2000-Ma0100-alpha0100
#+END_SRC

#+RESULTS:
[[file:multi-view-dust-bfield-b75-L2000-Ma0100-alpha0100.pdf]]

**** TODO Distribution in planitude-alatude plane
+ Finding R_c and R_90 can be done much like in the Spitzer dataset
+ 
** TODO B-field combined with drag
+ This is the version in the small Larmor radius limit
+ [2018-01-14 Sun 20:10] But I am doing the resolved Larmor radius version first, and then may come back to this

#+BEGIN_SRC python :eval no :tangle dust_bdrag_ode.py
  import numpy as np
  from scipy.integrate import odeint
...
** TODO Resolving the gyro-radius
#+BEGIN_SRC python :eval no :tangle dust_blorentz_ode.py
  import numpy as np
  from scipy.integrate import odeint
...
#+BEGIN_SRC python :eval no :tangle dust-lorentz-3d.py
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
  import seaborn as sns
...
#+BEGIN_SRC sh :results file
python dust-lorentz-3d.py
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-3d.pdf]]


#+BEGIN_SRC python :eval no :tangle dust-lorentz-stream.py
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
  import seaborn as sns
  ...
  Hd, [xe, ye, ze] = np.histogramdd(np.stack((xx, yy, zz), axis=1),
                                    bins=(100, 100, 100), 
...
Run this in a terminal because it takes 20 mins!

#+BEGIN_SRC sh :results file :eval no
python dust-lorentz-stream.py 15.0 20.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-b15-L2000-Ma0100.jpg]]


*** Convert npz cubes to fits format
#+BEGIN_SRC python :eval no :tangle convert-npz-cube-to-fits.py
  import sys
  import glob
  import numpy as np
  from astropy.io import fits
...
#+BEGIN_SRC sh :results silent
python convert-npz-cube-to-fits.py dust-lorentz-stream-b05-L2000-Ma0100-alpha0100
#+END_SRC

#+BEGIN_SRC sh :results silent
python convert-npz-cube-to-fits.py dust-lorentz-stream-b75-L2000-Ma0100-alpha0100
#+END_SRC

#+BEGIN_SRC sh :results silent
python convert-npz-cube-to-fits.py dust-lorentz-stream-b45-L0200-Ma0020-alpha0010
#+END_SRC

#+BEGIN_SRC sh :results silent
python convert-npz-cube-to-fits.py dust-lorentz-stream-b45-L2000-Ma0100-alpha0400
#+END_SRC


*** Selected streamlines in 3D
#+BEGIN_SRC python :eval no :tangle dust-lorentz-stream-3d.py
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
...
#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 45.0 5.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b45-L0500.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 45.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b45-L1000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 15.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b15-L1000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 75.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b75-L1000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 255.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b255-L1000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 75.0 20.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b75-L2000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 45.0 20.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b45-L2000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 15.0 20.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b15-L2000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 75.0 50.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b75-L5000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d.py 90.0 50.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-b90-L5000.jpg]]



*** Selected streamlines, but with turbulent component

#+BEGIN_SRC python :eval no :tangle dust-lorentz-stream-3d-turb.py
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
  from mpl_toolkits.mplot3d import Axes3D
...
#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 75.0 20.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b75-L2000-Ma0100-Z+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 75.0 20.0 10.0 -0.25
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b75-L2000-Ma0100-Z-025.jpg]]
...
#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 15.0 20.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b15-L2000-Ma0100-Z+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 15.0 20.0 10.0 -0.25
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b15-L2000-Ma0100-Z-025.jpg]]
...
#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 45.0 20.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b45-L2000-Ma0100-Z+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 90.0 20.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b90-L2000-Ma0100-Z+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 0.0 20.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b00-L2000-Ma0100-Z+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 0.0 5.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b00-L0500-Ma0100-Z+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 0.0 5.0 50.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b00-L0500-Ma0500-Z+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 0.0 10.0 50.0 -0.5
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b00-L1000-Ma0500-Z-050.jpg]]

The above three are the ones I think we will use:
+ 15, 45, 75 degrees
+ Lorentz force factor of 20
  + Should give close to the zero Larmor limit for most streamlines
+ M_A = 10
  + For instance, v_A = 2 and v_\infty = 20
+ /Update/ [2018-01-13 Sat] Actually, think that maybe the 0 and 90 degree ones would be best to show
  + 


**** Some runs with non-zero drag
#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 90.0 50.0 10.0 0.0 2.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b90-L5000-Ma0100-Z+000-alpha0200.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 0.0 20.0 10.0 0.0 0.25
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b00-L2000-Ma0100-Z+000-alpha0025.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 0.0 20.0 10.0 0.0 0.5
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b00-L2000-Ma0100-Z+000-alpha0050.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 0.0 20.0 10.0 0.0 1.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b00-L2000-Ma0100-Z+000-alpha0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 0.0 20.0 10.0 0.0 2.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b00-L2000-Ma0100-Z+000-alpha0200.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 5.0 20.0 10.0 0.0 1.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b05-L2000-Ma0100-Z+000-alpha0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 5.0 20.0 10.0 0.0 0.5
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b05-L2000-Ma0100-Z+000-alpha0050.jpg]]
...
#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b-5-L2000-Ma0100-Z+000-alpha0050.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 5.0 20.0 10.0 0.0 0.75
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b05-L2000-Ma0100-Z+000-alpha0075.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 5.0 50.0 10.0 0.0 0.75
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b05-L5000-Ma0100-Z+000-alpha0075.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 2.0 50.0 10.0 0.0 0.75
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b02-L5000-Ma0100-Z+000-alpha0075.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 182 50.0 10.0 0.0 0.75
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b182-L5000-Ma0100-Z+000-alpha0075.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 15.0 20.0 10.0 0.0 0.75
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b15-L2000-Ma0100-Z+000-alpha0075.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 15.0 50.0 10.0 0.0 0.75
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b15-L5000-Ma0100-Z+000-alpha0075.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 15.0 200.0 10.0 0.0 0.75
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b15-L20000-Ma0100-Z+000-alpha0075.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 15.0 50.0 10.0 0.0 1.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b15-L5000-Ma0100-Z+000-alpha0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 30.0 50.0 10.0 0.0 1.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b30-L5000-Ma0100-Z+000-alpha0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 45.0 50.0 10.0 0.0 1.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b45-L5000-Ma0100-Z+000-alpha0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 45.0 50.0 10.0 0.0 0.5
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b45-L5000-Ma0100-Z+000-alpha0050.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 60.0 50.0 10.0 0.0 1.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b60-L5000-Ma0100-Z+000-alpha0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 60.0 50.0 10.0 0.0 2.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b60-L5000-Ma0100-Z+000-alpha0200.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 75.0 50.0 10.0 0.0 2.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b75-L5000-Ma0100-Z+000-alpha0200.jpg]]

**** Extra runs that we probably won't use

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 75.0 10.0 10.0 
#+END_SRC

#+RESULTS:

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 75.0 20.0 1.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b75-L2000-Ma0010.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 75.0 20.0 30.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b75-L2000-Ma0300.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py -75.0 20.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b-75-L2000-Ma0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 255.0 20.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b255-L2000-Ma0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 75.0 50.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b75-L5000-Ma0100.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 15.0 10.0 5.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b15-L1000-Ma0050.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 45.0 10.0 5.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b45-L1000-Ma0050.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turb.py 45.0 10.0 10.0 
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turb-b45-L1000-Ma0100.jpg]]


*** Selected streamlines, but with varying z
#+BEGIN_SRC python :eval no :tangle dust-lorentz-stream-3d-turbz.py
  import sys
  import numpy as np
  from matplotlib import pyplot as plt
  from mpl_toolkits.mplot3d import Axes3D
...
#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 75.0 20.0 10.0 -0.25
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b75-L2000-Ma0100-Y-025.jpg]]
...
#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 15.0 20.0 10.0 -0.5
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b15-L2000-Ma0100-Y-050.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 15.0 20.0 10.0 -0.75
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b15-L2000-Ma0100-Y-075.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 15.0 20.0 10.0 -0.9
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b15-L2000-Ma0100-Y-090.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 45.0 20.0 10.0 -0.5
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b45-L2000-Ma0100-Y-050.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 90.0 20.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b90-L2000-Ma0100-Y+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 0.0 20.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b00-L2000-Ma0100-Y+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 0.0 50.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b00-L5000-Ma0100-Y+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 0.0 5.0 10.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b00-L0500-Ma0100-Y+000.jpg]]

#+BEGIN_SRC sh :results file
python dust-lorentz-stream-3d-turbz.py 0.0 5.0 50.0 0.0
#+END_SRC

#+RESULTS:
[[file:dust-lorentz-stream-3d-turbz-b00-L0500-Ma0500-Y+000.jpg]]
...
+ Material from Weingartner papers
  + Weingartner & Draine (2001)
    ...
    + Mainly concentrates on interstellar field, but does do an example of H II regions
      + Although does not give charge results - only heating/cooling
...
#+BEGIN_SRC sh :results file
python wave_xyprime.py 0.2 1.0 wilkinoid
#+END_SRC

#+RESULTS:
[[file:wave_xyprime-A020-N10-wilkinoid.pdf]]

#+BEGIN_SRC sh :results file
python wave_xyprime.py 0.1 2.0 wilkinoid
#+END_SRC

#+RESULTS:
[[file:wave_xyprime-A010-N20-wilkinoid.pdf]]

#+BEGIN_SRC sh :results file
python wave_xyprime.py 0.04 5.0 wilkinoid
#+END_SRC

#+RESULTS:
[[file:wave_xyprime-A004-N50-wilkinoid.pdf]]

#+BEGIN_SRC sh :results file
python wave_xyprime.py 0.1 2.0 ancantoid-xi080-beta000500
#+END_SRC

#+RESULTS:
[[file:wave_xyprime-A010-N20-ancantoid-xi080-beta000500.pdf]]
...
#+BEGIN_SRC sh :results verbatim
  for shape in wilkinoid cantoid-beta002000 ancantoid-xi080-beta000500; do
...
#+call: run-save-wave(A=0.2, N=1)

#+RESULTS:
: wilkinoid-wave-A020-N10.json
: cantoid-beta002000-wave-A020-N10.json
: ancantoid-xi080-beta000500-wave-A020-N10.json
...
#+call: run-save-wave(A=0.04, N=5)

#+RESULTS:
: wilkinoid-wave-A004-N50.json
: cantoid-beta002000-wave-A004-N50.json
: ancantoid-xi080-beta000500-wave-A004-N50.json
...
#+call: run-save-wave(A=0.1, N=2)

#+RESULTS:
: wilkinoid-wave-A010-N20.json
: cantoid-beta002000-wave-A010-N20.json
: ancantoid-xi080-beta000500-wave-A010-N20.json
...
#+BEGIN_SRC sh :results file
  python wave-R90-vs-Rc.py A020-N10 \
         cantoid-beta002000 \
         ancantoid-xi080-beta000500 \
         wilkinoid 
#+END_SRC
...
#+RESULTS:
[[file:wave-R90-vs-Rc-A020-N10.pdf]]

#+BEGIN_SRC sh :results file
  python wave-R90-vs-Rc.py A010-N20 \
         cantoid-beta002000 \
         ancantoid-xi080-beta000500 \
         wilkinoid 
#+END_SRC
...
#+RESULTS:
[[file:wave-R90-vs-Rc-A010-N20.pdf]]
...
#+BEGIN_SRC sh :results file
  python wave-R90-vs-Rc.py A004-N50 \
         cantoid-beta002000 \
         ancantoid-xi080-beta000500 \
         wilkinoid 
#+END_SRC
...
#+RESULTS:
[[file:wave-R90-vs-Rc-A004-N50.pdf]]
...
* Jane Conacyt project 2018

** Personal
#+BEGIN_EXAMPLE
  1. Informacion Relevante del Participante
  ,* sugiero que me envien una linea sobre el area en que son expertos y cual seria su papel en el proyecto. *

  2. Logros (3 principales)
  ,* por favor inventen algo *
#+END_EXAMPLE

+ William Henney is an expert on radiation transfer, atomic/molecular physics, and magnetohydrodynamics, particularly as applied to photoionized and photodissociated regions.  He has contributed to the inclusion of advection effects and time dependence in the Cloudy microphysics code \citep{Henney:2005b, Henney:2007a, Ferland:2013a}.  He has also worked extensively on theory and observations of bow shocks in the Orion Nebula.
+ Logros principales
  1. Dynamics of photoevaporation flows from globules and cirmcumstellar disks
  2. Structure and dynamics of the Orion Nebula
  3. Scattering of emission lines by dust
** Project 
:PROPERTIES:
:EXPORT_FILE_NAME: conacyt-4-jane
:EXPORT_OPTIONS: num:nil toc:nil author:nil
:END:
*** Computer equipment
**** Macbook Pro (custom config)
+ Laptop 15 inch
+ Intel core i7 quad core 3.1 GHz
+ 16 GB RAM
+ 1 TB SSD
+ 80199 MXN
...
**** Macbook Pro
+ Laptop 15 inch
+ Intel core i7 quad core 2.9 GHz
+ 16 GB RAM
+ 512 GB SSD
+ 65199 MXN
**** iMac
+ Desktop all-in-one
+ Intel core i5 quad core 3.5 GHz
+ 32 GB RAM
+ 1 TB SSD
+ Retina 5K screen
+ 61999 MXN
...
*** Antecedentes
+ Bow shocks form from the supersonic interaction of a stellar wind with an external stream of dust and plasma
  + The external stream can be due to the star's own motion, or due to turbulence and champagne flows in the interstellar medium
+ Hundreds of stellar bow shocks, associated with hot OB stars, have
  recently been discovered in mid-infrared surveys
  \citep{Kobulnicky:2016a}
+ This adds to dozens known to be associated with cooler red supergiant and AGB stars \citep{Cox:2012a}, and nearly a hundred associated with young stars and proplyds in the Orion Nebula \citep{Gutierrez-Soto:2015a}
+ For many types of bow shocks, the easiest way to observe them is via their dust continuum emission at infrared wavelengths
+ But the dust can also influence the dynamics and structure of the bow:
  + When the stellar wind is relatively weak, the dust absorption of stellar radiation becomes the dominant force, and, instead of a bow shock, a more gradual bow wave is formed \citep{van-Buren:1988a}
  + If the collisional gas-grain coupling is sufficiently weak, then the radiation force can separate the dust from the rest of the plasma, forming a gas-free dust wave \citep{Ochsendorf:2014a}
  + When grains are separated from the gas, magnetic fields nearly always dominate the grain dynamics, even when the magnetic pressure is smaller than the thermal gas pressure.
*** Meta
+ Study the coupling and uncoupling between dust grains, gas, and magnetic field in the presence of a strong radiation field, and its effect on the shapes and emission properties of stellar bow shocks
*** Metodología
1. Delineation of regimes of parameter space
   - For different types of stars (MS OB stars, B supergiants, M supergiants), find regions of the external stream parameter space (density, velocity) that are occupied by wind bow shocks, radiation bow waves, radiation bow shocks, and decoupled dust waves.
2. Calculation of dust dynamics in bow shocks and dust waves, subject to radiative, magnetic, and collisional drag forces. 
   - I. Weak coupling limit: fixed background plasma flow.  Explicit integration of grain equation of motion along streamlines, using generic ODE solver.
     - A. Guiding center approximation
     - B. Resolved Larmor radius
   - II. Strong coupling: back-reaction on plasma. Incorporation of dust dynamics in radiation MHD code \citep{Henney:2009a, Arthur:2011a}
3. Synthesize maps of dust emission and calculate distributions of bow shape parameters: planarity and alatude \citep{Tarango-Yong:2018a}. 
4. Comparison with properties of observed bow shocks from Spitzer
   MIPS, WISE, and Herschel surveys.
*** Calendario 
+ Year 1: M1, M2-I-A, M2-I-B
+ Year 2: M2, M4
+ Year 3: M2-II
