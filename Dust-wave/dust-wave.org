...
* Utility library for projecting a general shape
+ [ ] I need to find a better home for this
...
#+BEGIN_SRC python :tangle bowproject.py :eval no
  import numpy as np
  from scipy.optimize import brentq
  from scipy.misc import derivative

  def omega(func_R, theta):
      """Find omega = (1 / R) dR / d theta
  Note that theta may be an array
    
  """
      def log_R(theta):
          return np.log(func_R(theta))
      return derivative(log_R, theta, dx=1.e-6)



  # * Example shapes 

  def parabola_R_theta(theta):
      """This is the special parabola with Rc = 2"""
      return 1.0 / (1.0 - np.cos(theta))

  def parabola_omega_true(theta):
