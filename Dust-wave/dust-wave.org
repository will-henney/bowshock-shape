...
*** COMMENT Strong coupling case
+ [X] Add in the i-front trapping
  + Column density of shell: \(R n\)
  + Shell thickness: \(h = \frac38 R (c_{s} / v)^{2} \simeq \frac38 R v_{10}^{-2}\)
  + Shell EM: \( (R n)^{2} / h = \frac83 v_{10}^{2} n^{2} R\)
  + Compare recombinations in shell with ionizing flux: \( f = \alpha_{B} . EM . 4\pi r^{2} / S_{*}\)
  + \( f = \frac{32\pi}{3} v_{10}^{2} n^{2} R^{3} (\alpha_{B} /  S_{*})\)
