...
#+BEGIN_SRC python :tangle bow_projection.py :eval no
  ...
  def theta_0(inc, func_R, *args_for_func_R):
      ...
      def _f(theta):
          """Function that is zero at theta = theta0"""
          om = omega(theta, func_R, *args_for_func_R)
      ...
      if np.abs(inc) > th_inf - np.pi/2:
          ...
          # Root could be anywhere in range 0 -> th_inf, but we set the
          # lower limit higher than that to avoid some rare errors.
          # This should be alright unless R_c/R_0 < 0.1, which is not
          # true for any of the models I am interested in
          th1, th2 = 0.1*inc, th_inf
          # Make sure we do indeed bracket the root
          assert _f(th1)*_f(th2) <= 0.0, f"Unbracketed root: {_f(th1)}, {_f(th2)}"
          # And use Brent's method to find the root
          return brentq(_f, th1, th2)
