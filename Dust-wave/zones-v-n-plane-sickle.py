...
stardata = [
    [10.0, 0.63, 0.0066, 1e-4, axes],
]

# Velocities in units of km/s (1 km/s -> 100 km/s)
logvgrid = np.linspace(0.0, 2.5, 800)
# Densities in units of 1 pcc  (0.01 -> 1e5)
logngrid = np.linspace(-3.3, 6.3, 800)
# 2d versions of velocity and density grids
vv, nn = np.meshgrid(10**logvgrid, 10**logngrid)
...
for M, L4, eta, S49, ax in stardata:
    ...
    T0[absfrac + advecfrac > 1.0] = 100.0
    ...
    # Add box for measured proper motion of 96 +/- 40 km/s => 46 - 136
    ax.axvspan(46.0, 136.0, 0.0, 0.5, alpha=0.1)
    # Add box for typical densities of 1000 to 1e4 pcc
    ax.axhspan(1000.0, 1.0e4, 0.0, 0.5, color="g", alpha=0.1)

    ax.text(20.0, 1.0, r"B1.5V, $\eta = 0.007$, $R = 0.087$ pc", rotation=-34, fontsize="x-small")
