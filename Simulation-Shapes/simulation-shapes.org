...
+ [ ] Finally there is Meyer 2017 with magnetic fields
  ...
  + So plan is
    1. [ ] Trace the CD from the density images, following a contour
       - This gives the R(\theta), which can be plotted as \Delta(\mu)
       - Extrapolate to give a full shape
       - Use that to predict \Pi', \Lambda' as function of inclination
    2. [ ] Trace the 60 micron images to find \Pi', \Lambda' for the inclinations 30, 45, 60
