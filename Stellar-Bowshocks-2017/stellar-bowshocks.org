...
#+BEGIN_SRC python :eval no :tangle mipsgal-trace-arc.py
  ...
  for source_data in source_table[0:1]:
      ...
      papix = c.position_angle(cpix).to(u.degree)
      # Nominal PA from table
      pa0 = coord.Angle(source_data['PA'], unit=u.degree)
      # theta is angle from nominal axis, set to range [-180:180]
      theta = coord.Longitude(papix - pa0, wrap_angle=180*u.degree)
