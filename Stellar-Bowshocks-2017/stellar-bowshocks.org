...
#+BEGIN_SRC python :return figfile :results file
  ...
  fig, (ax, axr) = plt.subplots(2, 1, sharex=True, figsize=(6, 9))
  ...
  xgrid = np.linspace(0.0, 5.0)
  # x0 = np.log10(R0_median)
  ...
  # This does the Pearson r at the same time
  a, b, r, p, da = linregress(np.log10(tab[R0var][mgood].data), yvar[mgood])
  s = 'Linear regression: $y = m x + c$'
  s += '\n' + f'$m = {a:.2f} \pm {da:.2f}$, $c = {b:.2f}$'
  s += '\n' + f'Correlation coefficient: $r = {r:.2f}$'
  if p > 0.001:
