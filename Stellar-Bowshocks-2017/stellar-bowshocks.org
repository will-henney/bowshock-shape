...
**** TODO Galactic longitude and latitude
...
+ [ ] So I can recalculate A_K using the formula:
  + A_K = (0.893 \pm 0.090) (H - [4.5] + 0.1)
+ [ ] And then find the corrected stellar magnitude as:
  + H_0 = H - (1.55 \pm 0.08) A_K

**** TODO Multifactor pair plot of the non-shape parameters
...
+ /Things that we find:/
  1. Clearest correlation is between size and H_0, which is probably due to distance of the source
     + H band magnitudes of Trapezium, etc
       | Star | Sp Type |    H |  A_V |    M_H |
       |------+---------+------+-----+-------|
       | \theta^1 C | O7 V    | 4.63 |   1 | -3.58 |
       | \theta^1 B | B3 V    |  6.3 |   1 | -1.91 |
       | \theta^1 A | B0 V    |  5.8 |   1 | -2.41 |
       | \theta^1 D | B1.5 V  | 5.89 |   1 | -2.32 |
       | \theta^2 A | O9.5 IV | 4.97 | 0.2 | -3.08 |
       | \theta^2 B | B2-B5 V | 6.27 | 0.2 | -1.78 |
#+TBLFM: $5=$3 - 0.2 $4 - 8.01
    + Absolute magnitudes are calculated assuming A_H = 0.2 A_V and D = 400 pc
      + Distance modulus: 5 log10(D) - 5 = 8.01
    + So it seems -4 (early O) to -1.5 (mid-B) is a reasonable spread for the H-band absolute magnitude
      + So that is exactly a factor of 10 in luminosity, which seems reasonable
...
#+BEGIN_SRC python :return figfile :results file
  ...
  # Corregt H magnitude for extinction
