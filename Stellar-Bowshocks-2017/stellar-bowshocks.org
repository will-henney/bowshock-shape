...
**** TODO Test out log-log axes for these graphs
...
#+BEGIN_SRC python :results file :return figfile
  ...
  for ax, Rshape in zip(axes, ['R90', 'Rc', 'dR90', 'R90_sigma', R0var]):
      ...
      sns.boxplot(x=R0_label, y=tab[Rshape][mgood], order=R0_order, ax=ax, boxprops=dict(lw=0.0), showfliers=False, notch=dependent)
      if dependent:
  ...
  axes[2].set(ylabel='Asymmetry\n' + r'$|\Lambda_{+} - \Lambda_{-}|/ \Lambda$',
              ylim=[-0.01, None], yticks=[0, 0.2, 0.4, 0.6, 0.8])
  axes[3].set(ylabel='Point dispersion\n' + r'$\epsilon (\Lambda)$',
              ylim=[-0.01, None], yticks=[0, 0.2, 0.4, 0.6, 0.8])
  axes[4].set(ylabel='Bows size\n' + r"$R_0$, arcsec", yscale='log',
              ylim=[4.5, 190.0])
  axes[4].yaxis.set_major_locator(mticker.FixedLocator([5, 10, 100]))
  ...
  for iR0, R0_median in enumerate(R0_order):
      ...
      axes[-1].text(x, 0.92, f'$N = {np.sum(R0_label == R0_median)}$',
                    ha='center', va='top', transform=ax.transAxes,
                    fontsize='small'
      )
