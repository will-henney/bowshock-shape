...
+ Now I have discovered more tests for equality of dispersion
  ...
  + But the only one I can find a python routine for is [[https://en.wikipedia.org/wiki/Levene%2527s_test][Levene's test]]
    ...
    + Has the advantage that there are three different variants, which use different measures of the "location" (median, mean, or trimmed mean), which are better suited to different sorts of distributions
