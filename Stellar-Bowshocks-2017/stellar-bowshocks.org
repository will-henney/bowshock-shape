...
#+BEGIN_SRC python :return figfile :results file
  ...
  with sns.plotting_context(
      ...
      for m, c, axx in zip(masks, colors, [ax_y_R90, ax_n_R90]):
          ...
          axx.text(1.0, 0.5, fr'$N = {np.sum(m)}$',
                   fontsize=8, ha='right', va='top',
                   transform=axx.transAxes)
          # Calculate K-S statistic
          if c != colors[0]:
...
#+BEGIN_SRC python :return figfile :results file
  ...
  r, p = pearsonr(np.log10(tab[R0var][mgood].data), tab['R90'][mgood].data)
  s = f'$r = {r:.2f}$'
  if p > 0.001:
      s += '\n' + f'$p = {p:.4f}$'
  else:
...
Try a finer discretization of the R_0 axis into 8 bins
...
#+BEGIN_SRC python :results file :return figfile
  ...
  fig, axes = plt.subplots(3, 1, sharex=True, figsize=(3.5, 6))
  ...
  for ax, Rshape in zip(axes, ['R90', 'Rc', R0var]):
      m = mgood & np.isfinite(tab[Rshape])
      sns.boxplot(x=R0_label, y=tab[Rshape][m], order=R0_order, ax=ax)
  axes[0].set(ylabel=r'$R_{90} / R_{0}$')
  axes[1].set(ylabel=r'$R_{c} / R_{0}$', ylim=[None, 5.0])
  axes[2].set(ylabel=r'$R_0$', yscale='log', ylim=[None, 100.0])
  for iR0, R0_median in enumerate(R0_order):
      x = (iR0 + 0.5) / len(R0_order)
      axes[-1].text(x, 0.96, f'$N = {np.sum(R0_label == R0_median)}$',
                    ha='center', va='top', transform=ax.transAxes,
                    fontsize='x-small'
      )
...
#+BEGIN_SRC python :results file :return figfile
  ...
  #R0_label = (2*np.round(logR0/2, 1)).astype('U3')
  H0_order = sorted(set(H0_label), key=float)

  for ax, Rshape in zip(axes, ['R90', 'Rc', 'Hmag']):
      m = mgood & np.isfinite(tab[Rshape])
      sns.boxplot(x=H0_label, y=tab[Rshape][m], order=H0_order, ax=ax)
  axes[0].set(ylabel=r'$R_{90}$')
  axes[1].set(ylabel=r'$R_{c}$', ylim=[None, 5.0])
  axes[2].set(ylabel=r'$H_0$')
  for iH0, H0_median in enumerate(H0_order):
      x = (iH0 + 0.5) / len(H0_order)
      axes[-1].text(x, 0.96, f'$N = {np.sum(H0_label == H0_median)}$',
                    ha='center', va='top', transform=ax.transAxes,
                    fontsize='x-small'
      )
  axes[-1].set(xlabel=r"Sub-sample median $H_0$", )
  sns.despine()
  fig.tight_layout()
  fig.savefig(figfile)
#+END_SRC
...
#+BEGIN_SRC sh :results output
  FIGFILES='mipsgal-r0-r0-plus-dPA-edited.pdf mipsgal-Rc-R90-zoom-annotated.pdf mipsgal-Rc-R90-thumbnails.pdf mipsgal-pairplot.pdf mipsgal-Rc-R90-Mag.pdf mipsgal-Rc-R90-R0.pdf mipsgal-Rc-R90-candidates.pdf mipsgal-Rc-R90-environment.pdf mipsgal-Rc-R90-vs-Orion.pdf mipsgal-Rc-R90-vs-Herschel.pdf p-value-histogram.pdf mipsgal-boxplot-Rc-R90-versus-R0.pdf'
  date
  pwd
...
+ [ ] Get the McGrath & Meyer (2006) paper
