...
+ [X] And add in the wing asymmetry factor |R_{90+} - R_{90-}| / R_90
  + This is about 0.2
+ [ ] Try tests of heteroscedasticity
...
#+BEGIN_SRC python :results file :return figfile
  ...
  fig, axes = plt.subplots(4, 1, sharex=True, figsize=(3, 7))
  ...
  for ax, Rshape in zip(axes, ['R90', 'Rc', 'dR90', R0var]):
      ...
      if Rshape != R0var:
          ...
          AD, levelsAD, pAD = anderson_ksamp(
              ...
              midrank=False)
          ax.text(0.1, -0.04, f'A-D $p = {pAD:.3f}$',
                  ha='left', va='top', transform=ax.transAxes,
                  fontsize='x-small'
          )
  ...
  axes[3].set(ylabel=r"$R_0$, $''$", yscale='log', ylim=[None, 200.0])
...
#+BEGIN_SRC python :results file :return figfile
  ...
  for ax, Rshape in zip(axes, ['R90', 'Rc', 'dR90', 'Hmag']):
  ...
  axes[1].set(ylabel=r'$R_{c} / R_{0}$', ylim=[0.0, 5.0])
  axes[2].set(ylabel=r'$|R_{90+} - R_{90-}|/ R_{90}$', ylim=[-0.01, None], yticks=[0, 0.2, 0.8])
  axes[3].set(ylabel=r'$H_0$, mag', ylim=[4.5, 13.9], yticks=[6, 9, 12])
...
**** False discovery rate revisited [2017-04-07 Fri]
...
+ Some of the anti-Colquhoun points made are
  ...
  2. That Colquhoun is assuming point nulls and two-sided tests, whereas in practice people often use compound nulls and one-sided tests
     ...
     - A compound null can be something like "A is no better than B", that is, "difference is less than or equal to zero"
  3. That his "FDP \ge 30% for p < 0.05" is not as watertight as he thinks
     - It would be bigger if the prior probability of the null were < 50%
       ...
       + But he is right that it would be odd to be testing /only/ hypotheses that we already are convinced of
     - Supposedly things are different with one-sided tests and compound nulls, but I haven't really looked into this
