...
#+BEGIN_SRC python :return figfile :results file
  ...
  for m, alpha, cmap, shade, gamma, ls in zip(
      ...
      sns.kdeplot(otab['Rc out'].data, otab['R90'].data, 
                  n_levels=8, cmap='Reds_d', linewidths=1, norm=PowerNorm(gamma),
                  linestyles=ls, 
                  #levels=[0.05, 0.1, 0.3, 0.7, 0.9, 1.0, 1.2, 1.4, 1.6],
                  bw=(0.12, 0.08),
                  shade=False, shade_lowest=False, ax=ax, alpha=alpha)
...
#+BEGIN_SRC python :results silent
  ...
  def plot_rsg_image_fit(source='AlphaOri',
                         pmin=1, pmax=99,
      ...
      fits_pattern = ('RSG/OriginalData/MESS_PPHOT/'
                      + f'{source.upper()}*[-_]70_*pixfrac*.fits')
      fitsfile = glob.glob(fits_pattern)[0]
      hdulist = fits.open(fitsfile)
      hdu = hdulist['image']
      hdu160 = fits.open(fitsfile.replace('70', '160'))['image']
      ...
      fig.savefig(f'RSG/{source.lower()}-imageplot.pdf')
      plt.close(fig)
...
+ If we use criterion that we must be able to see the shell at theta=90, then we could also argue for
  - [-] W Pic - although gets a bit speckly around the edge
  - [-] Chi Cyg - although so big it may not fit on image
  - [-] Omicron Cet - although we would have to ignore the fainter outer shell
+ And also maybe
  - [-] RT Vir - which has a very pointy shell
  - [-] X Her - with a two-headed appearance
  - [-] R Cas - faint around the edge
  - [-] EP Aqr - although multiple shells?
  - [-] CW Leo - although again very big, and only at 160 micron
  - [-] W Aql - although maybe faint at sides
+ Ones we definitely can't do
  + TX Psc - one-sided and pointy
  + T Mic - very irregular, multiple strands
  + Theta Aps - too faint at sides
  + W Pic - too faint at sides
  + W Ori - too faint full stop
  + R Scl - small and diffuse sides
