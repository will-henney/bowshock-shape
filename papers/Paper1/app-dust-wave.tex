...
\begin{figure}
  ...
  \caption{Dust-gas coupling for an on-axis (purely radial) trajectory
    with \(\alpha\drag = 0.5\). (a)~Grain velocity (in units of
    \(v_\infty\)) and radius (in units of \(R_0\)) versus time. (b)~Phase
    space trajectory. Grain spirals in on point
    \((x, u) = (\alpha\drag^{-1}, 0)\).}
...
\(\approx R_0 / \alpha\drag \) and be swept in again for another try.  This
behavior is illustrated in Figure~\ref{fig:dust-coupling-1d}, where it can be seen that 
