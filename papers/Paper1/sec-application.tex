...
fits are relatively robust to removing points, resulting in little
variation between the subsamples.

In the left side, for each source, the fit is done using all the ds9
marks, while in the middle and the right we show two variations
removing a fraction of the marks, but always keeping at least four
points. 177-341 is an example where the measurements only change very
slightly from one variation to other. While with 168-328 the changes
are a little more noticeable. LV4 is a case where the most external
points are crucial in the determination of the radius of curvature.
