...
\begin{figure}
  ...
  \caption{Example of a family of conic sections, all with the same
    ...
    \(\Pi\) can be achieved for the same quadrics by sliding them along
    the \(x\)-axis, which will also change the axis scales since these
    are normalized by \(R_0\).}
...
\end{figure*}
We now apply the machinery of \S~\ref{sec:projection} to find the projected shape of a quadric bow on the plane of the sky.
...
in section.  First of all, the intrinsic 3D shape of the shell is given by:
\begin{align}
x &= a\Cos(t)+x_0 \\ 
y &= b\Sin(t)\cos\phi \\
z &=  b\Sin(t)\sin\phi
