...
for the \(xy\) coordinates of the quadrics (in the \(\phi = 0\)plane, and
with the star at the origin) as a function of \(t = [0, \pi]\) is then
\begin{gather}
  \label{eq:par-xy}
  \begin{aligned}
    x &= x_0 + \sigma a \Cos(t) \\ 
...
\begin{equation}
  ...
  \Sin(t), \Cos(t) = \begin{cases}
    \sin(t), \cos(t) & \text{ellipse}\\
    \sinh(t), \cosh(t) & \text{hyperbola}
  \end{cases}
\end{equation}
...
are the sphere (\(T_q = 1\)) and the paraboloid (\(T_q = 0\)).  The
set of parameters \((T_q, a, x_0)\) are then sufficient to
characterize the star/bow combination, where \(a\) is the quadric
scale and \(x_0\) is its center displacement from the star.

But, we can also characterize the star/bow by \((R_0, \Pi, \Lambda)\), where
...
from oblate to prolate to hyperboloid, as illustrated in
Figure~\ref{fig:conics-family}.
