...
\item Looping over a grid of 50 position angles, \(\text{PA}_k\), within
  ...
  \(C_1\).
\item For each of the points \(C_{k,\text{peak}}\) and \(C_{k,\text{mean}}\), determine the radial separation from the central source, \(R_k = |C_k - C_0|\) and the angle from the bow shock axis about the central source \(\theta_k = \text{PA}(C_k, C_0) - \text{PA}_0\).  These are plotted in the upper left panels of  Figure~\ref{fig:mipsgal-examples}. 
