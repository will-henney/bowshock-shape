...
\begin{figure*}
  ...
  \caption{As Fig~\ref{fig:herschel-arc-fits}, but for a further 8
    ...
    more diffuse, weak, and/or asymmetric. For CW~Leo only (top-left),
    the right-hand panel shows a grayscale image of the \SI{160}{\um}
    rather than \SI{80}{\um} emission.}
...
classes, according to the overall shape of the extended emission at
\SI{70}{\um}: ``fermata~\textfermata'', or arcs (Class~I);
``eyes~\faEye'' (Class~II); ``rings~{\Large \(\odot\)}'' (Class~III), and
``irregular~\staveXXV'' (Class~IV).  Of these, only the Class~I
sources clearly correspond to bow shocks, which represent 23 out of 50
total sources detected with extended emission.  For our sample, we
select the 7 Class~I sources where the shape parameters can be
unambiguously measured, and which are shown in
Figure~\ref{fig:herschel-arc-fits}.  For some of the remaining Class~I
sources (Fig.~1 of \citealp{Cox:2012a}), the bow shock becomes too
diffuse around the ``sides'', so that \(R_{90}\) cannot be reliably
determined (examples include \(\theta\)~Aps and )
