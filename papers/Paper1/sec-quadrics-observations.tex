...
through the interstellar medium (runaway stars), or are interacting
with a local bulk flow, such as the champagne flow from a nearby
\hii{} region.
...
\item For each of the points \(C_{k}\), determine the radial
  ...
  the upper left panels of Figure~\ref{fig:mipsgal-examples}.
...
nonetheless some problem is apparent that casts doubt on the
reliability of the results.  For example, a double-shell structure to
the bow shock that leads to large differences between the ``peak'' and
``mean'' methods, or point sources near to the bow shock that
interfere with the tracing procedure.
...
\bigskip
Only sources with a 3-star or higher rating are used in the following analysis, which amount to approximately half (227 out of 471) of all the MIPSGAL arc sources. 
