...
bolometric luminosity, \(L\), is offset by an increase in the
effective temperature, \(T_{\text{eff}}\), which shifts the peak of
the stellar spectrum farther away from the \(H\)~band, resulting in
...
this may be affected by various factors:
\begin{enumerate}[1.]
\item Distance, \(d\), since for a constant physical size the angular
  size will vary as \(R_0 \propto d\).
\item Momentum-loss ratio, \(\beta\), between the two flows that form the
  bow shock.  All other things being equal, we have
  \(R_0 \propto \beta^{1/2}\).  If the environment flow is constant and the OB
  star wind has mass-loss rate \(\dot{M}\) and terminal velocity
  \(V_\infty\), then \(\beta \propto \dot{M}{V_\infty}\).  Empirical and theoretical
  studies of hot star winds (e.g., \citealp{Puls:1996a}) imply
  \(\dot{M}{V_\infty} \sim L^{1.88} R^{-1/2} \sim L^{1.80}\), where the final
  step applies only to main-sequence stars and again uses the
  relations of \citet{Eker:2015a}.  
\end{enumerate}
