\message{ !name(quadrics-bowshock.tex)}%\RequirePackage{amsmath}
...
\message{ !name(app-shape-parameters.tex) !offset(-107) }

\section{Analytic derivation of thin-shell bow shapes}
...
order \(1\%\) even for \(\beta\) as large as \(0.5\) with \(k = 0\).  The
leading term in the relative error scales as \((\beta / (k + 2))^2\), so the
approximation is even better for smaller \(\beta\) and larger \(k\).
...
\end{equation}
Note that, unlike the case for \(\Pi\) and \(\Lambda\),
equation~\eqref{eq:anacantoid-theta-inf} does \emph{not} reduce to
...
cantoid since there is no wind in the backward hemisphere (see
...
\(\beta \to 0\), implying that \(\theta_\infty = \pi\), or, in other words, that
the far wings are asymptotically parallel to the symmetry axis, as is
the case for the paraboloid (App.~\ref{app:parabola}).  In the case of
the wilkinoid, however, the behavior is cubic in the wings,
...
\message{ !name(quadrics-bowshock.tex) !offset(-137) }
