...
not possible and a shocked shell must form (see Paper~I's Fig.~6).
...
\(R\grain(\thm; b)\), but instead must be found by minimizing
...
which is discussed in detail in \S~4 and
Appendix~C of \citet{Tarango-Yong:2018a}.  In that
...
Fig.~3 of \citealp{Tarango-Yong:2018a}). We
...
\begin{figure}
  ...
  \caption{Dust wave formed by action of radiation forces on grains
    ...
    one with \(b < R\starstar\) (lower) are shown schematically by
    black lines with arrows.  The orientation of the magnetic field
    lines is shown in blue.  The grayscale image shows the resultant
...
trajectories are very similar to those of the non-magnetic case
