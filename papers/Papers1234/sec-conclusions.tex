...
\begin{figure*}
  ...
  \caption{Example empirical determination of planitude and alatude
    ...
    (\textit{a})~outer edge, (\textit{b})~ridge of maximum emission,
    and (\textit{c})~inner edge.  For each panel, the dark-colored
    ...
    \(\pm\ang{75}\) of the axis.  The subset of points used in the
    final iteration is marked in red.  In panels \textit{b} and
    \textit{c} the iterations converge immediately, but in panel
    \textit{a} the iterations stably oscillate between two slightly
    different solutions.  (\textit{d})~Location of the converged
    circle fits in the projected planitude--alatude plane.  Each
    solution has two values of the alatude \(\Lambda'\), corresponding to
    \(R_{90}^+\) and \(R_{90}^-\).}
