...
\begin{figure*}
  ...
  \caption{Drag force as a function of gas--grain slip velocity,
    \(w\), for different values of the grain potential,
    \(\phi = U / k T\).  Contributions from proton and electron Coulomb
    (electrostatic) drag, as well as Epstein (solid-body) drag are
    indicated.  Inset shows \(\phi\) as a function of the radiation
    parameter, \(\Xi = P_{\mathrm{rad}} / P_{\mathrm{gas}}\) on a
    log--linear scale.}
...
If the radiation field is sufficiently strong, then the collisional coupling between grains and gas will break down.  
