...
Examples of the each of these two behaviors are illustrated in
...
grain's approach it initially follows an equilibrium drift phase
trajectory (lower left panel) along the \(f\rad - f\drag = 0\)
contour, with a very slight deceleration.  Then, when it reaches the
rip point, \(R = R_\dag\), it suddenly experiences an unbalanced outward
radiation force (blue region of phase space in
Fig.~\ref{fig:phase-space-trajectories}). The grain's inward momentum
carries it to the point \(R\min\), which is only slightly inside
\(R_\dag\), before it is expelled at roughly twice the inflow speed.
However, after moving outward, it finds itself in a drag-dominated
region of phase space (red in the figure), and so recouples to the
inflowing gas stream.  The recoupling is gradual at first, as the
grain's outward motion is slowed and it begins to move inward again,
but is completed suddenly once \(w\) falls below \SI{10}{km.s^{-1}} in
what we term \textit{snap back}. The net result is the grain is back
on the same phase track that it came in on, and so repeats the cycle
indefinitely. 

The grain trajectory can be seen to follow a limit cycle, in which 

whether or not
the incoming stream velocity lies in drag regime IV
(Table~\ref{tab:fdrag-regimes}) with , which
produces the unstable drift region identified in
Figure~\ref{fig:gas-grain-drag-photoionized}.
