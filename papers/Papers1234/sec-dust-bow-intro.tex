...
which is approximately\footnote{%
  ...
  helium and its corresponding electrons gives a correction to this of
  order 5\%.  The required modifications when the stream is neutral
  are discussed later.  } %
...
\begin{equation}
  \label{eq:fdrag-epstein}
  f\drag \approx
  \begin{cases}
    \text{if \(\phi^2 \ll 1\) and \(w_{10} < 1\):} & w_{10}\, f_* \\
    \text{if \(\phi^2 \ll 1\) and \(w_{10} > 1\):} & w_{10}^2\, f_* \\
  \end{cases}
  \ ,
\end{equation}
where \(w_{10} = w / \SI{10}{km.s^{-1}}\).
...
electron Coulomb drag gives a second peak of similar strength, but at
\(w \approx \SI{430}{km.s^{-1}}\).
...
radiative force from a star.  We are now in a position to see how
gas--grain drag will modify this picture.  From
equations~\eqref{eq:dust-rad-force} and~\eqref{eq:fstar}, we can write
the radiation force acting on a grain as
\begin{equation}
  \label{eq:frad-Xi}
  f\rad = \Qp\, \Xi\, f_* \ ,
\end{equation}
...
\end{equation}
where the last expression corresponds to the optically thin limit and
pure ionized hydrogen.  The grain potential \(\phi\) is also primarily
determined by \(\Xi\), as shown in Appendix~\ref{sec:cloudy-models-dust}
and the inset graph of Figure~\ref{fig:gas-grain-drag-photoionized},
but with a slow dependence, which can be approximated\footnote{%
  There is a slight secondary dependence on the grain composition and
  stellar spectrum.  The relationship given in
  eq.~\eqref{eq:phi-vs-Xi} is appropriate for graphite grains and for
  stellar effective temperatures in the range \SIrange{20}{30}{kK}.
  For hotter stars than this, \(\phi\) should be multiplied by a further
  factor of \(1.5\), while for silicate grains it should be divided by
  \(1.5\).  } %
as
\begin{equation}
  \label{eq:phi-vs-Xi}
