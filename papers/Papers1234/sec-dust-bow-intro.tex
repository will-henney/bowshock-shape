...
\begin{figure*}
  ...
  \caption{Regions of stream parameter space \((v, n)\) where dust
    ...
    dominated, while dust waves below the lines are inertia dominated.
  }
...
We will refer to this as an \textit{inertia-dominated dust wave}.  From
...
as a \textit{drag-dominated dust wave}.
...
drag-dominated dust waves, or \(\Rmin \approx R\starstar\) for
inertia-dominated dust waves.  We write the field strength in terms of
the Alfvén speed,
\begin{equation}
  \label{eq:alfven}
...
around the field lines, plus a sliding of the guiding center
along the field lines, which is governed by the radiation and drag
forces (we ignore the slow \(\bm{f} \times \bm{B}\) drift across the field
lines).  In the opposite limit, \(r\B / \Rmin \gg 1\), magnetic coupling
is so weak that the results of the previous sections are scarcely
modified.
