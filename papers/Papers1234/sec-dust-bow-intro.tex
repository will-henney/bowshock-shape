...
entire bow shock shell, or whether the ionization front will be trapped
within it.  The number of hydrogen recombinations per unit time per
unit area in a fully ionized shell is \(\mathcal{R} = \alphaB n\shell^2 h\shell\),
while the flux of ionizing photons incident on the inner edge of the
shell is \(\mathcal{F} = S / 4 \pi R_0^2\).  Assuming photoionization equilibrium, the hydrogen photoabsorption optical depth of the shell is
\begin{equation}
  \label{eq:ion-tau-gas}
  \tau\gas = - \ln(1 - \mathcal{R} / \mathcal{F}) \ .
\end{equation}
The column density of the shocked shell can be found, for example, from equations~(10) and~(12) of \citet{Wilkin:1996a} in the limit \(v/V \to 0\) (Wilkin's parameter \(\alpha\)) and \(\theta \to 0\).  This yields
\begin{equation}
