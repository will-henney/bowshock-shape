...
terms of \(\phi\) and \(w_{10} = w / \SI{10}{km.s^{-1}}\).
...
dependence, which can be approximated\footnote{%
...
grains.  Thus, from equation~\eqref{eq:fdrag-epstein}, the radiative
force can be balanced by Epstein drag if \(w_{10} \sim 0.1\), leading to
a small drift velocity, \(w\drift \sim \SI{1}{km.s^{-1}}\), of the grains
with respect to the gas.

As a dust grain
approaches a star, the radiation parameter \(\Xi\) will increase as
\(R^{-2}\)
