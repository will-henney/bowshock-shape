...
trapped within it.  The number of hydrogen recombinations\footnote{%
  ...
  \(\alphaB = \num{2.6e-13}\, (T/\SI{e4}{K})\, \si{cm^3.s^{-1}}\), is
  used.} %
per unit time per unit area in a fully ionized shell is
\(\mathcal{R} = \alphaB n\shell^2 h\shell\), while the flux of
hydrogen-ionizing photons (\(h \nu > \SI{13.6}{eV}\)) incident on the
inner edge of the shell is \(\mathcal{F} = S / 4 \pi R_0^2\), where
\(S\) is the ionizing photon luminosity of the star.  Assuming
photoionization equilibrium, the hydrogen photoabsorption optical
depth of the shell is
\begin{equation}
  \label{eq:ion-tau-gas}
  \tau\gas = - \ln(1 - \mathcal{R} / \mathcal{F}) \ ,
\end{equation}
so long as \(\mathcal{R} < \mathcal{F}\).  The column density of the
shocked shell can be found, for example, from equations~(10) and~(12)
of \citet{Wilkin:1996a} in the limit \(v/V \to 0\) (Wilkin's parameter
...
the shell density is \(n\shell = M_0^2 n\), where \(M_0 = v / \sound\)
is the isothermal Mach number of the external stream.  We will assume
...
in Table~\ref{tab:stars}.  The dust opacity is approximately constant
