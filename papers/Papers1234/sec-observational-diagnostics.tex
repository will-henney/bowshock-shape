...
\subsection{Why the two mass-loss methods differ}
...
\(\Pi = R_{\text{c}} / R_0\)\citep{Tarango-Yong:2018a}.  This is
illustrated in the inset to Figure~\ref{fig:H-versus-ell}, from which
we find
\begin{equation}
  \label{eq:h-R0}
...
\subsection{Shared properties of candidates for radiation support}
...
narrow range of spectral type\footnote{%
  However, this is not particularly statistically significant.  For
...
theory developed in Paper~I.  Radiation-supported bows are predicted
...
radiation-supported candidates.\footnote{%
  ...
  \(p = (\frac{5}{20})^4 = 0.0039\) for the null hypothesis.}  From
equation~\eqref{eq:h-R0} this means they also should have thicker than
average shells.  This is consistent with our finding from Paper~I's
\S~3.3 that the shell tends to be broad in the radiation-supported bow
wave regime.  Furthermore, if the larger than average \(H/R_0\) were
to be taken account in the calculation of \(\eta\shell\)
(eq.~\eqref{eq:eta-shell}), then it would strengthen the case for
...
Correlations between observed parameters and with mass loss rates.
Pair plot from my jupyter notebook.

How both the UV opacity and the IR emissivity depend on the same VSG grains.

Effects of grain evolution.  RATD destroying larger grains.  How would
this effect the abundance of smaller grains? Calculate maximum grain
size for the \(U\) distribution of the K18 sample.  Do we see any
effects that seem to vary with \(U\)?
