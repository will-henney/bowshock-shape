...
#+END_EXAMPLE
* Revised version
** Using latexdiff to hilight changes
...
+ I had to edit the individual files to put ~\makeatletter~ ... ~\makeatother~ around the expanded .sty files.
+ But after that, they both compile with no problems
...
#+call: run-latexdiff(OPTIONS="--preamble=wjh-latexdiff-preamble.tex --append-context2cmd='label' --packages=amsmath,hyperref,siunitx --verbose --ignore-warnings --math-markup=0 --allow-spaces") :results output verbatim
...
we agree that a small preview would be useful here.  So we have added
a new section where we apply the methods to that bow shock and another
similar one.
