...
parser.add_argument(
    ...
    choices=("jorge_mir", "will_mir", "jorge_60", "jorge_45",
             "jorge_curv"), 
    default="will_mir",
...
def wf_2(c):
    """Calculate the algebraic distance between the 2D weighted points
    and the mean circle centered at c=(xc, yc) """
    Ri = calc_R(*c)
    return u*(Ri - Ri.mean())
...
obs_label = ['LV2', 'LV3', 'LV4', 'LV5', 'LV2b',
             '141-301', '176-341', '180-331']
...
for inn in innertype:
    ...
    for b, col in zip(beta, colors[:len(beta)]):
        ...
        for i, inc in enumerate(inclinations):
            ...
            c0 = 0, 0
            try:
		xim_fit, yim_fit = x2[(np.abs(np.degrees(np.arctan2(y2,x2)))) -tfit <= 0.], y2[(np.abs(np.degrees(np.arctan2(y2,x2)))) -tfit <= 0.]
            except IndexError:
		...
		c_fit, ier = leastsq(f_2, c0)
	    	...
	    	R0[i] = xim[0]
                Rc[i] = Ri_2.mean()  # The fit uses the bowshock data such that t<45 deg
...
plt.savefig("combined-shell-R0-Rc{}{:.0f}.pdf".format(suffix,tfit))
plt.clf()
